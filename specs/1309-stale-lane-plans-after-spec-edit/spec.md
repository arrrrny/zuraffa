...
- **FR-004**: After `zfa tdd plan` or `zfa tdd split` completes on a receipt-bearing feature, every lane plan file reflects the current behavior set in the spec, no stale requirement rows remain, and the lane plan modification times are at least the spec's modification time.
