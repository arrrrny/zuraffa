...
## 1. `zfa tdd verify-red` summary line (extends FR-009 contract)

Final stdout line on every code path. New optional `evidence` token on certified
reds:

```
verify-red: behavior=<id> classification=<class> certified=<bool> feature=<feature>
verify-red: behavior=<id> classification=assertion certified=true feature=<feature> evidence=<identity>
```

- `<identity>`: single-line failing-assertion identity extracted from the
  transcript (spaces allowed, no newlines), or `none` when the transcript
  carries no parseable identity. Omitted entirely for non-`assertion` classes
  (existing lines byte-identical → no consumer breaks).
- Exit codes unchanged: 0 ⇔ honest red certified (assertion class).

## 2. `zfa tdd verify-red` stdout detail lines (human+agent readable)

On a certified red, before the summary line:

```
   red-evidence: <identity>
```

Printed only when an identity was extracted; absent otherwise.

## 3. Cycle-log red entries (`specs/<feature>/tdd/cycle-log.md`)
...
## 4. Generated widget subject stub (target-project source contract)
...
## 5. Generated widget test harness (unchanged shape, re-anchored comments)
