...
- [ ] T001 (TDD, red) Write `test/commands/format_runner_1506_test.dart`
...
- [ ] T002 (green) Implement `lib/src/core/format/format_runner.dart`:
...
- [ ] T004 (green) Wire `EntityCommand` to `FormatRunner`: constructor
  injection (`formatRunner`), `_runFormat()` delegates with scope
  `[fixedEntityOutput]`, surfaces the single warning when present.
  Run T001+T003 → GREEN.
- [ ] T005 (refactor) Dedupe fixture helpers only if duplication is
  real (recorder type shared via the existing test conventions);
  re-run T001+T003.

## Phase B — Non-behavioral (enforcement surface)

- [ ] T006 AGENTS.md hard rule: enforce `dart pub get --no-example`
  before every formatter invocation (fresh-clone guard) — FR-4.
- [ ] T007 `.github/agents/surgical-pr-fix.agent.md`: pub-get
  precondition added to the `dart format <touched files>` verify step —
  FR-4.
- [ ] T008 CI note: verify `.github/workflows/ci.yaml` format job order
  (already `pub get --no-example` → `format --set-exit-if-changed`);
  record as verified-no-change in verification.md — FR-4.

## Phase C — Verification

- [ ] T009 Targeted lanes: `dart test
  test/commands/format_runner_1506_test.dart
  test/commands/entity_format_scope_1506_test.dart` green.
- [ ] T010 `dart analyze lib test` — zero new issues vs baseline
  (0 errors / 112 pre-existing infos).
- [ ] T011 `dart format` on every touched file (idempotence check) +
  full-suite spot: `dart test test/commands/` green.
- [ ] T012 Write [tdd/verification.md](tdd/verification.md): test-first
