...
- [x] T001. [behavior: U4] Fingerprint service: `GenReuseFingerprint.compute`
...
- [x] T002. [behavior: U1] Traces migration with a SIGNATURE row
...
- [x] T003. [behavior: U2] Traces migration with a NO-SIGNATURE row
...
- [x] T004. [behavior: U3] Drift + progressed subject refuses reuse:
      exit 1, verdict refused, `--> fix: zfa tdd reset <feature>`.
      Traces FR-3 / AS-3.
- [x] T005. [behavior: U5] Drift fires once per change: after a
      drift-driven regeneration the next gen is `reused`.
      Traces FR-2 / AS-4.
- [x] T006. [behavior: U6] Unchanged routing and legacy records keep
      byte-identical reuse (no fingerprint / equal fingerprint).
      Traces FR-4 / FR-5 / AS-5.
- [x] T007. Wire the gate into gen `_generate` (created records arm the
...
- [x] T008. Non-behavioural: `dart analyze` clean on touched files,
