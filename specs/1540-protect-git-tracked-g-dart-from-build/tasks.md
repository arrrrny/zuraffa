...
- [ ] T001 Create `lib/src/core/generation/tracked_generated_output_guard.dart`
...
- [ ] T002 Write `test/core/generation/tracked_generated_output_guard_test.dart`
...
- [ ] T003 Implement guard behavior: `git -C <root> ls-files -z` parse
  (NUL-separated, project-relative), suffix filter (`.g.dart`,
  `.zorphy.dart`), byte capture, restore, failure buckets. **[GREEN]**
- [ ] T004 Wire `BuildCommand.run()`: capture snapshot pre-build (non-dry-run
...
- [ ] T005 Write `test/commands/build_command_tracked_outputs_test.dart`
...
- [ ] T006 Extend `verifyDeclaredPartsOrFail` (signature unchanged): on
...
- [ ] T007 Gate tests in `test/commands/build_command_tracked_outputs_test.dart`:
...
- [ ] T008 Extend `RefactorPassesResult` with `refusalReason` (nullable) +
...
- [ ] T009 Print `refusalReason` in `refactor_command.dart` misfire-stop
  block (distinct from the timeout branch).
- [ ] T010 Write `test/plugins/tdd/bug_1540_refactor_tracked_restore_test.dart`
...
- [ ] T011 Write `docs/hand-authored-g-dart-placeholders.md`: pattern
...
- [ ] T012 Cross-artifact consistency pass: spec criteria ↔ test-list rows ↔
  tasks; fix drift (`/speckit.analyze` equivalent, recorded in
  `tdd/verification.md`).
- [ ] T013 `dart analyze` clean on changed files; `dart format lib test`;
  full fast-tier suite green; slow-tier refactor suites green.
- [ ] T014 Record evidence in `tdd/verification.md` (red run, green run,
