...
| The SAME run is still inside the window (review of #1578) | The resolution is deferred to the first `filter` read, so any resolution state produced after `buildContext` lands first — but the window does NOT close inside the run: the ensure writes the pubspec at the end of the run and deliberately never spawns `pub get`, and `_resolve` reads `package_config.json`, which that write does not change. A colliding entity generated in that window emits no hide → `ambiguous_export` (an error, never tolerated by the #942 refusal) → `generation-error`. Accepted residual: it clears on the next run after the user follows the receipt's `Run pub get to re-resolve`; the alternative is the undefined_hidden_name gate-red this spec fixes for every non-colliding entity |
