...
- [x] **T001** (P1) `test/plugins/tdd/services/build_relevance_test.dart`
  ...
  SC-001..SC-003]
- [x] **T002** (P1) `test/plugins/tdd/make_command_1587_build_skip_test.dart`
...
- [x] **T003** (P1) same file — generation writes an annotated
  (`@Zorphy`) dart file → `build` present (unchanged), #737/#942
  tolerance paths intact. [FR-003, SC-002]
- [x] **T004** (P2) same file — generation changes `pubspec.yaml` →
  `build` present (config churn never skips). [FR-001, SC-003]
- [x] **T005** (P2) same file — hashless-certified-red make still runs
  the live drift check (dedup fails open) and the live build. [FR-005]
- [x] **T006** (P1) `test/plugins/tdd/make_command_1587_dedup_test.dart`
  ...
  SC-004]
- [x] **T007** (P1) same file — subject hash differs (hand-implemented
  subject) → live drift check runs (fail-open note absent). [FR-005,
  SC-005]
- [x] **T008** (P2) same file — green entry after the last red → live
...
- [x] **T101** (P1) `lib/src/plugins/tdd/services/build_relevance.dart`
  — fingerprint + `canSkipTerminalBuild` (FR-001).
- [x] **T102** (P1) `lib/src/plugins/tdd/models/generation_plan.dart` —
  `GenerationStep.buildSkipped` optional field (default false) (FR-002).
- [x] **T103** (P1) `lib/src/plugins/tdd/services/pipeline_runner.dart` —
...
- [x] **T104** (P1) `lib/src/plugins/tdd/models/cycle_entry.dart` —
  render `note:` line for skipped build steps in the generation block
  (FR-002).
- [x] **T105** (P1) `lib/src/plugins/tdd/commands/make_command.dart` —
  ...
  FR-005/FR-007).
- [x] **T106** (P2) docs touch: the make command's header comment block
  records the #1587 scheduling contract (FR-007).

## 3. Verification

- [x] **T201** full targeted suites green (both new files + existing
