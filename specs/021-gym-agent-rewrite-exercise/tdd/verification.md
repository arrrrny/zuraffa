...
| `dart analyze` | 111 issues / 23 errors — **byte-identical to the master baseline** (`diff` of sorted output empty); all pre-existing in `zikzak_session/` + `examples/mcp_demo/` (missing git submodule content, documented since spec 038); `.gym/**` is excluded from analysis by `analysis_options.yaml` |
