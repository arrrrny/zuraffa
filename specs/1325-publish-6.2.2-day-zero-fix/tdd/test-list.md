...
| B3 | `dart pub publish --dry-run` on the master tree reports 0 errors (warnings permitted per the FR-3 warnings-only policy — a warnings-only exit is the accepted pass shape) and the upload set includes `lib/src/core/benchmark/benchmark_contract.dart` | FR-3 / SC-3 | `dart pub publish --dry-run` |
