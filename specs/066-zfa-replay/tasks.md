...
- [ ] T001 Create `ReplayCommand` as a skeleton in
...
- [ ] T002 [P] [U1] [U2] Write the failing history-parsing tests FIRST in
  ...
  for imports only)
- [ ] T003 [P] Implement `ReplayHistory` in
  ...
  (depends on T002 red)
- [ ] T004 [P] [U3] [U4] [U5] Write the failing integrity/replayability tests
  ...
  skipped) — observe red
- [ ] T005 [P] Implement the integrity stage + replayability on
...
- [ ] T008 [P] [U7] [U9] Write the failing gen-replay + report tests FIRST in
...
- [ ] T009 [P] Implement the gen stage + report shaping in
...
- [ ] T010 [P] [U8] [U9] Write the failing verify-replay tests FIRST: recorded
...
- [ ] T012 [A5] [A1] Write the failing command e2e tests FIRST in
  ...
  observe red
- [ ] T013 Implement the command body in `replay_command.dart`: arg parsing →
...
- [ ] T014 [A2] [A3] [A4] Write the failing mutation-catch e2e tests: SC2 —
...
- [ ] T015 Make T014's catches pass through the wired command (fix any drift
...
- [ ] T016 [P] [U10] [A6] Write the failing event-log tests FIRST in
...
- [ ] T017 [P] Implement the NDJSON writer (in
  ...
  red)
- [ ] T018 [A7] Write the failing top-level surface tests: `zfa replay
...
- [ ] T019 Implement the delegating top-level command + registration (path
...
- [ ] T021 Run `dart analyze` — zero new issues vs the 47-issue baseline;
  run `tools/run_tests_chunked.sh` — all chunks pass; record real
  pass/fail counts for tdd/verification.md
- [ ] T022 Run `dart format .` — zero-format-diff gate; clean fixture/build
