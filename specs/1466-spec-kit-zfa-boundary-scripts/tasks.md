...
- [x] T001 [US1] Create `.specify/scripts/bash/tests/harness.sh`: `t_case`
...
- [x] T002 [US2] Create `.specify/scripts/bash/tests/run_tests.sh`: discover
...
- [x] T003 [P] [US1] RED: write `tests/test_sync_behaviors.sh` with S1
...
- [x] T004 [P] [US1] GREEN: implement the three cases; suite passes.
- [x] T005 [P] [US1] RED: add S4 (malformed ID skipped with stderr warning,
...
- [x] T006 [P] [US1] GREEN: implement S4–S6; file passes 6/6.

## Phase 3.3 — read-tdd-profile.sh suite (US1)

- [x] T007 [P] [US1] RED: write `tests/test_read_profile.sh` with P1 (dart
...
- [x] T008 [P] [US1] GREEN: implement P1–P3.
- [x] T009 [P] [US1] RED: add P4 (missing file → non-zero + stderr; no
...
- [x] T010 [P] [US1] GREEN: implement P4–P6; file passes 6/6.

## Phase 3.4 — read-cycle-evidence.sh suite (US1)

- [x] T011 [P] [US1] RED: write `tests/test_read_evidence.sh` with E1 (RED/
  ...
  Run → red evidence.
- [x] T012 [P] [US1] GREEN: implement E1–E3.
- [x] T013 [P] [US1] RED: add E4 (missing file → non-zero + stderr), E5
  (text mode summary lists entries; --json parseable by jq when present).
  Run → red for new cases.
- [x] T014 [P] [US1] GREEN: implement E4–E5; file passes 5/5.

## Phase 3.5 — tick-behavior-task.sh suite (US1)

- [x] T015 [P] [US1] RED: write `tests/test_tick_behavior.sh` with T1 (ticks
...
- [x] T016 [P] [US1] GREEN: implement T1–T3.
- [x] T017 [P] [US1] RED: add T4 (duplicate markers → first ticked, warning
  ...
  red for new cases.
- [x] T018 [P] [US1] GREEN: implement T4–T7; file passes 7/7.

## Phase 3.6 — Shellcheck gate (US2)

- [x] T019 [US2] Extend `run_tests.sh`: after suites, run
...
- [x] T020 [US3] Write `.specify/scripts/bash/README.md`: purpose of the four
...
- [x] T021 Run full 1466 suite (`run_tests.sh`) + shellcheck; record actual
  counts in `tdd/verification.md`.
- [x] T022 Regression: run 1444 suite
  (`specs/1444-spec-kit-boundary-scripts/tdd/tests/run_all_tests.sh`) — must
  stay 20/20 (SC-6).
- [x] T023 Constraint audit: `git diff --stat` confirms zero changes under
