...
- [ ] T114 [F1] Register the slice plugin's commands in the speckit zuraffa extension: add the `slice` entries (command + capability aliases) to `.specify/extensions/zuraffa/extension.yml` `provides.commands` with their command `.md` files, following the existing entries' shape. Prove done: `dart test test/cli/standard/extension_command_parity_test.dart` passes.
- [ ] T115 [F2] Pin A10's modified-flag with a value assertion: in `test/plugins/slice/slice_list_inspect_test.dart`, assert the edited file's full line (e.g. `contains('product_view.dart — modified')`) and that an untouched file prints `— unmodified`, so the substring overlap can no longer hide a dead flag. Prove done: re-run deliberate mutant M2 (inspect always prints `unmodified`) and confirm `dart test test/plugins/slice/slice_list_inspect_test.dart` now fails; restore the mutant and confirm green.
- [ ] T116 [F3] Pin A9's file count: assert the actual per-slice count (e.g. `contains('${expectedCount} files')` computed from the cut result data or the manifest), not `contains('files')`. Prove done: re-run deliberate mutant M5 (`list` prints `0 files`) and confirm the file fails; restore and confirm green.
- [ ] T117 [F4] Replace the vacuous U20/U21/U26 dedup pins with assertions that can fail: assert against an ordered LIST projection of the walk output (e.g. `nodes.keys.toList()` compared with an explicit expected-path list) and for U26 assert the manifest's `files` list contains the shared usecase exactly once (a List, where duplication is representable). Prove done: `dart test test/plugins/slice/engine/import_graph_walker_test.dart` passes, and a hand-made duplicate insertion into the expected list fails the test (spot-check).
- [ ] T118 [F6] Remove the real-clock expectation from A9's date check: pin the created date from the manifest the test itself wrote (read `slice.yaml` after cut) instead of `DateTime.now()`. Prove done: `dart test test/plugins/slice/slice_list_inspect_test.dart` passes with no `DateTime.now()` in the file.
- [ ] T119 [F7] Pin the e2e merge count exactly: assert on the merge report's copied list via the manifest re-read or match `'1 file(s) copied back'` with word boundaries (e.g. `contains(RegExp(r'(?<!\d)1 file\(s\) copied back'))`). Prove done: `dart test test/plugins/slice/slice_e2e_test.dart` passes and a temporary 11-file copy mutant fails it (spot-check).
- [ ] T120 [F10] Assert the step marker in the progress test: add `expect(output, contains(RegExp(r'\[=+>\s*\] \d+/\d+')))` (or the exact step line) so the name "started/step/completed" is honest. Prove done: `dart test test/plugins/slice/slice_polish_test.dart` passes.
- [ ] T121 [F11] Tighten U9 to `equals(['GetProductUseCase', 'FetchSettingsUseCase'])` (order-stable per the analyzer's traversal). Prove done: `dart test test/plugins/slice/engine/service_locator_analyzer_test.dart` passes.
- [ ] T122 [F12] Replace the `isNotEmpty` assertions with value assertions (`equals('Slice')`, `equals('1.0.0')`) and clean up the three leaked temp dirs in `slice_plugin_registration_test.dart` (tearDown deletion). Prove done: `dart test test/plugins/slice/slice_plugin_registration_test.dart` passes; no `createTempSync` without cleanup.
- [ ] T123 [F9] Extract the shared fixture-copy/runner/teardown boilerplate into `test/plugins/slice/helpers/` (e.g. a `sliceTestHarness()` factory), and refactor the 10 integration test files to use it. Prove done: `dart test test/plugins/slice/` passes unchanged (145 tests).
