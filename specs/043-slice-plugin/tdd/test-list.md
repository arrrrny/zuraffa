...
| A10 | `slice inspect` shows every file with ownership classification and modified-since-cut status                                           | US3-S2, FR-012, FR-010    | example | PENDING |      |
| A11 | A two-entry cut contains both pages' dependency trees with shared dependencies included exactly once                                   | US4-S1, FR-011            | example | PENDING |      |
| A12 | A usecase shared by both entries appears once in the manifest with one DI registration                                                 | US4-S2, FR-011            | example | PENDING |      |
| A13 | `--depth view` includes view/controller/state and no presenter, usecases, or entities                                                  | US5-S1, FR-002            | example | PENDING |      |
| A14 | `--depth feature` (default) adds presenter/usecases/domain interfaces/entities but no data implementations                             | US5-S2, FR-002            | example | PENDING |      |
| A15 | `--depth full` additionally includes repository implementations, datasources, and providers                                            | US5-S3, FR-002            | example | PENDING |      |
