...
3. Clone and validate (prefer plain `git clone` — `gh repo clone` resolves
   the repository via the GitHub GraphQL API and intermittently fails with
   `GraphQL: Could not resolve to a Repository`; see
   https://github.com/arrrrny/zuraffa/issues/1508). Replace `<your-username>`
   with your own GitHub account (the owner of the exported repo) and note that
   the repo is private, so git needs credentials — run `gh auth setup-git`
   once if you have not already:
   ```bash
   # for private repos ensure git creds are set up, e.g. `gh auth setup-git`
   git clone https://github.com/<your-username>/<auto-generated-repo-name>.git /tmp/slice_github
   cd /tmp/slice_github
   flutter pub get
   flutter analyze
   ```
