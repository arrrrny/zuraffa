...
**Note:** The `--vpcs` flag creates files in `presentation/pages/{entity}/` to better organize multi-page applications.
...
**Note:** State is managed by the Controller, not the Presenter. When using the `--state` flag with `--vpcs`:
