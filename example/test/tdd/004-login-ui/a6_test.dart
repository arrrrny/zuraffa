...
import 'package:example/tdd/004-login-ui/a6_subject.dart' as subject;

void main() {
  group('A6 (AC-6)', () {
    testWidgets('A6 — the \'Sign in\' button is disabled', (tester) async {
      ...
      {
        final buttonFinder = find.widgetWithText(ElevatedButton, t.auth.signIn);
        expect(
          buttonFinder,
          findsOneWidget,
          reason: 'the scenario asserts the t.auth.signIn control exists',
        );
        expect(
          tester.widget<ElevatedButton>(buttonFinder).onPressed,
          isNull,
          reason: 'the scenario asserts the t.auth.signIn control is disabled',
        );
      }
