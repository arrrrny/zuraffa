...
import 'package:example/tdd/004-login-ui/a3_subject.dart' as subject;

void main() {
  group('A3 (AC-3)', () {
    testWidgets('A3 — the app shows \'Sign in\'', (tester) async {
    ...
    testWidgets(
      'A3 — expansion locale de renders every keyed surface (issue #965)',
      (tester) async {
        ...
        await tester.pumpAndSettle();
        expect(
          find.text(t.auth.signIn),
          findsOneWidget,
          reason: 'the keyed surface t.auth.signIn must render under de',
        );
      },
    );
  });
