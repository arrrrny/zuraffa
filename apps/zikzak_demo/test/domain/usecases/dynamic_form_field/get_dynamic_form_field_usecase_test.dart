...
import 'package:zikzak_demo/src/domain/usecases/dynamic_form_field/get_dynamic_form_field_usecase.dart';
...
void main() {
  late GetDynamicFormFieldUseCase useCase;
  late GetDynamicFormFieldUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetDynamicFormFieldUseCase(repository);
    throwingUseCase = GetDynamicFormFieldUseCase(throwingRepository);
  });
  group('GetDynamicFormFieldUseCase', () {
    ...
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<DynamicFormField>(
          filter: Eq(DynamicFormFieldFields.id, tDynamicFormField.id),
        ),
      );
      expect(result.isSuccess, true);
      expect(
        result.getOrElse(() => throw (Exception('not success'))),
        equals(tDynamicFormField),
      );
    });
