...
import 'package:zikzak_demo/src/domain/usecases/chat_session/get_chat_session_usecase.dart';
...
void main() {
  late GetChatSessionUseCase useCase;
  late GetChatSessionUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetChatSessionUseCase(repository);
    throwingUseCase = GetChatSessionUseCase(throwingRepository);
  });
  group('GetChatSessionUseCase', () {
    final tChatSession = ChatSessionMockData.sampleChatSession;
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<ChatSession>(
          filter: Eq(ChatSessionFields.id, tChatSession.id),
        ),
      );
