...
import 'package:zikzak_demo/src/domain/usecases/url_listing/get_url_listing_usecase.dart';
...
void main() {
  late GetUrlListingUseCase useCase;
  late GetUrlListingUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetUrlListingUseCase(repository);
    throwingUseCase = GetUrlListingUseCase(throwingRepository);
  });
  group('GetUrlListingUseCase', () {
    final tUrlListing = UrlListingMockData.sampleUrlListing;
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<UrlListing>(
          filter: Eq(UrlListingFields.id, tUrlListing.id),
        ),
      );
