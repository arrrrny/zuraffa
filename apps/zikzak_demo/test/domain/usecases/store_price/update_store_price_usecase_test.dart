...
import 'package:zikzak_demo/src/domain/usecases/store_price/update_store_price_usecase.dart';
...
void main() {
  late UpdateStorePriceUseCase useCase;
  late UpdateStorePriceUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = UpdateStorePriceUseCase(repository);
    throwingUseCase = UpdateStorePriceUseCase(throwingRepository);
  });
  group('UpdateStorePriceUseCase', () {
    final tStorePrice = StorePriceMockData.sampleStorePrice;
    test('should call repository.update and return result', () async {
      final result = await useCase.call(
        UpdateParams<String, StorePricePatch>(
          id: tStorePrice.depotId,
          data: StorePricePatch(),
        ),
      );
