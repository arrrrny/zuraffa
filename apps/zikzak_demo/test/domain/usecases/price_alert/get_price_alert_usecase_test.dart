...
import 'package:zikzak_demo/src/domain/usecases/price_alert/get_price_alert_usecase.dart';
...
void main() {
  late GetPriceAlertUseCase useCase;
  late GetPriceAlertUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetPriceAlertUseCase(repository);
    throwingUseCase = GetPriceAlertUseCase(throwingRepository);
  });
  group('GetPriceAlertUseCase', () {
    final tPriceAlert = PriceAlertMockData.samplePriceAlert;
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<PriceAlert>(
          filter: Eq(PriceAlertFields.id, tPriceAlert.id),
        ),
      );
