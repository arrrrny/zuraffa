...
import 'package:zikzak_demo/src/domain/usecases/connected_account/get_connected_account_usecase.dart';
...
void main() {
  late GetConnectedAccountUseCase useCase;
  late GetConnectedAccountUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetConnectedAccountUseCase(repository);
    throwingUseCase = GetConnectedAccountUseCase(throwingRepository);
  });
  group('GetConnectedAccountUseCase', () {
    ...
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<ConnectedAccount>(
          filter: Eq(ConnectedAccountFields.id, tConnectedAccount.id),
        ),
      );
      expect(result.isSuccess, true);
      expect(
        result.getOrElse(() => throw (Exception('not success'))),
        equals(tConnectedAccount),
      );
    });
