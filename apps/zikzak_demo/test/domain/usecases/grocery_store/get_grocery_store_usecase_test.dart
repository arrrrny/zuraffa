...
import 'package:zikzak_demo/src/domain/usecases/grocery_store/get_grocery_store_usecase.dart';
...
void main() {
  late GetGroceryStoreUseCase useCase;
  late GetGroceryStoreUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetGroceryStoreUseCase(repository);
    throwingUseCase = GetGroceryStoreUseCase(throwingRepository);
  });
  group('GetGroceryStoreUseCase', () {
    final tGroceryStore = GroceryStoreMockData.sampleGroceryStore;
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<GroceryStore>(
          filter: Eq(GroceryStoreFields.id, tGroceryStore.id),
        ),
      );
      expect(result.isSuccess, true);
      expect(
        result.getOrElse(() => throw (Exception('not success'))),
        equals(tGroceryStore),
      );
    });
