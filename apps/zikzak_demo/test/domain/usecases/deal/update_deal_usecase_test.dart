...
import 'package:zikzak_demo/src/domain/usecases/deal/update_deal_usecase.dart';
...
void main() {
  late UpdateDealUseCase useCase;
  late UpdateDealUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = UpdateDealUseCase(repository);
    throwingUseCase = UpdateDealUseCase(throwingRepository);
  });
  group('UpdateDealUseCase', () {
    final tDeal = DealMockData.sampleDeal;
    test('should call repository.update and return result', () async {
      final result = await useCase.call(
        UpdateParams<String, DealPatch>(id: tDeal.id, data: DealPatch()),
      );
