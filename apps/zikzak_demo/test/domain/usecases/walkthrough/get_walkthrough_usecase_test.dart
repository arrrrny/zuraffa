...
import 'package:zikzak_demo/src/domain/usecases/walkthrough/get_walkthrough_usecase.dart';
...
void main() {
  late GetWalkthroughUseCase useCase;
  late GetWalkthroughUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetWalkthroughUseCase(repository);
    throwingUseCase = GetWalkthroughUseCase(throwingRepository);
  });
  group('GetWalkthroughUseCase', () {
    final tWalkthrough = WalkthroughMockData.sampleWalkthrough;
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<Walkthrough>(
          filter: Eq(WalkthroughFields.id, tWalkthrough.id),
        ),
      );
