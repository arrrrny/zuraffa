...
import 'package:zikzak_demo/src/domain/usecases/chat_message/update_chat_message_usecase.dart';
...
void main() {
  late UpdateChatMessageUseCase useCase;
  late UpdateChatMessageUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = UpdateChatMessageUseCase(repository);
    throwingUseCase = UpdateChatMessageUseCase(throwingRepository);
  });
  group('UpdateChatMessageUseCase', () {
    final tChatMessage = ChatMessageMockData.sampleChatMessage;
    test('should call repository.update and return result', () async {
      final result = await useCase.call(
        UpdateParams<String, ChatMessagePatch>(
          id: tChatMessage.id,
          data: ChatMessagePatch(),
        ),
      );
