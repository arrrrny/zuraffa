...
import 'package:zikzak_demo/src/domain/usecases/barcode/get_barcode_usecase.dart';
...
void main() {
  late GetBarcodeUseCase useCase;
  late GetBarcodeUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetBarcodeUseCase(repository);
    throwingUseCase = GetBarcodeUseCase(throwingRepository);
  });
  group('GetBarcodeUseCase', () {
    final tBarcode = BarcodeMockData.sampleBarcode;
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<Barcode>(filter: Eq(BarcodeFields.value, tBarcode.value)),
      );
