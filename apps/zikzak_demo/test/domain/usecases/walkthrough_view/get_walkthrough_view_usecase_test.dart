...
import 'package:zikzak_demo/src/domain/usecases/walkthrough_view/get_walkthrough_view_usecase.dart';
...
void main() {
  late GetWalkthroughViewUseCase useCase;
  late GetWalkthroughViewUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetWalkthroughViewUseCase(repository);
    throwingUseCase = GetWalkthroughViewUseCase(throwingRepository);
  });
  group('GetWalkthroughViewUseCase', () {
    ...
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<WalkthroughView>(
          filter: Eq(WalkthroughViewFields.id, tWalkthroughView.id),
        ),
      );
      expect(result.isSuccess, true);
      expect(
        result.getOrElse(() => throw (Exception('not success'))),
        equals(tWalkthroughView),
      );
    });
