...
import 'package:zikzak_demo/src/domain/usecases/metric_detail/get_metric_detail_usecase.dart';
...
void main() {
  late GetMetricDetailUseCase useCase;
  late GetMetricDetailUseCase throwingUseCase;
  ...
  setUp(() {
    ...
    useCase = GetMetricDetailUseCase(repository);
    throwingUseCase = GetMetricDetailUseCase(throwingRepository);
  });
  group('GetMetricDetailUseCase', () {
    final tMetricDetail = MetricDetailMockData.sampleMetricDetail;
    test('should call repository.get and return result', () async {
      final result = await useCase.call(
        QueryParams<MetricDetail>(
          filter: Eq(MetricDetailFields.name, tMetricDetail.name),
        ),
      );
      expect(result.isSuccess, true);
      expect(
        result.getOrElse(() => throw (Exception('not success'))),
        equals(tMetricDetail),
      );
    });
