...
| U-1134-a6 | test/plugins/tdd/commands/view_command_skin_contract_slots_test.dart | command | a malformed Skin Contract slot (an unknown slot in the contract) refuses BEFORE any write — exit 1, the subject byte-untouched (errors-are-an-API) | FR-001 | RED → GREEN (proved, this session) |
| U-1134-a7 | test/plugins/tdd/commands/view_command_skin_contract_slots_test.dart | command | width-defined slots (tablet/desktop) are reachable in the generated `_resolveSlot` — every slot the contract accepts can render (no dead layouts) | FR-001 | GREEN (review-fix pin, this session) |
...
| U-1134-t10 | test/plugins/tdd/services/typed_ledger_projection_test.dart | unit | rows with the same surface and kind but different proof semantics (absence state, state attribute, chain steps) stay SEPARATE — the row identity carries the semantics | FR-004 | GREEN (review-fix pin, this session) |
| U-1134-t11 | test/tdd/services/typed_platform_ledger_test.dart | unit | a NOT-DONE typed row never traces a slot, even when one of its provers exercised it (the aggregate honours the typed state) | FR-005 | GREEN (review-fix pin, this session) |
...
| U-1134-g5 | test/commands/skin_command_test.dart | command | `zfa skin grid`/`zfa skin table` refuse BY NAME with the usage exit code — the CLI owns the non-zero refusal (never a silent 0) | FR-007, US4-AC4 | GREEN (review-fix pin, this session) |
| U-1134-g6 | test/plugins/tdd/commands/plan_vocabulary_gate_1134_test.dart | command | the plan gate validates against built-ins PLUS the project's registered composites (`.zfa/ui/components/`) — a registered composite passes | FR-006, US4-AC2 | GREEN (review-fix pin, this session) |
| U-1134-g7 | test/plugins/tdd/commands/view_vocabulary_gate_test.dart | command | the view gate validates against built-ins PLUS the project's registered composites (`.zfa/ui/components/`) — a registered composite passes | FR-006, US4-AC3 | GREEN (review-fix pin, this session) |
