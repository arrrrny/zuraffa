...
import 'package:zuraffa/src/plugins/skin/vocabulary/composite_scaffolder.dart';
import 'package:zuraffa/src/plugins/skin/vocabulary/ui_node_registry.dart';
import 'package:zuraffa/src/plugins/skin/vocabulary/vocabulary_schema_exporter.dart';
  ...
  group('CompositeScaffolder', () {
    test(
      ...
      () async {
        ...
        final registration = File(
          '${tempDir.path}/.zfa/ui/components/offer_card.json',
        );
    ...
    test(
      ...
      () async {
        ...
        expect((export['components'] as Map).keys, contains('offer_card'));
      },
    );

    test(
      'payload referencing the composite validates (US-2 scenario 3)',
      () async {
        await CompositeScaffolder().scaffold(
          'OfferCard',
          projectRoot: tempDir.path,
    ...
    test(
      'reserved built-in names rejected with the reserved list (FR-007)',
      () async {
        expect(
          ...
          throwsA(
            isA<CompositeScaffoldException>().having(
              (e) => e.message,
              'message',
