...
void main() {
  group('McpSseServer', () {
    ...
    test('GET /sse opens an event-stream and emits the endpoint event', () async {
      final req = await client.getUrl(Uri.parse('http://127.0.0.1:$port/sse'));
      final res = await req.close();
      expect(res.statusCode, 200);
      expect(
    ...
    test('full POST round-trip: tools/list returns the registered tool definitions', () async {
      // Open SSE and buffer all events into a queue. Each event is
      // a chunk terminated by `\n\n` in the SSE wire format.
      final sseReq = await client.getUrl(Uri.parse('http://127.0.0.1:$port/sse'));
      final sseRes = await sseReq.close();
      final eventStream = sseRes.transform(utf8.decoder).expand((chunk) {
        return chunk
            .split('\n\n')
      ...
      final endpointEvent = await iter
          .moveNext()
          .timeout(const Duration(seconds: 2));
      ...
      final got = await iter
          .moveNext()
          .timeout(const Duration(seconds: 3));
      expect(got, isTrue);
      final msgData = iter.current;
      expect(msgData, contains('event: message'));
      final dataMatch = RegExp(r'^data: (.+)$', multiLine: true)
          .firstMatch(msgData)!;
      final responseJson = jsonDecode(dataMatch.group(1)!) as Map<String, dynamic>;
    ...
    test('POST /message dispatches a tools/call and streams the result', () async {
      final sseReq = await client.getUrl(Uri.parse('http://127.0.0.1:$port/sse'));
      final sseRes = await sseReq.close();
      final eventStream = sseRes.transform(utf8.decoder).expand((chunk) {
        return chunk
