...
void main() {
  ...
  test('A7: 100% attribution exits 0 with report + summary line', () async {
    ...
    final report =
        jsonDecode(await File(fx.auditReportPath).readAsString())
  ...
  test('A8: an unattributed file fails the audit BY NAME', () async {
    ...
    final report =
        jsonDecode(await File(fx.auditReportPath).readAsString())
            as Map<String, dynamic>;
    expect((report['unattributed'] as List).single, 'lib/mystery.dart');
  });

  test(
    'A9: removing a carve-out entry flips its file to unattributed',
    () async {
      await libFile('manual_ui.dart');
      await write(
        '.zfa/manifests/corpus-carveout.json',
  ...
  test('U32: no lib/ directory audits trivially green with files=0', () async {
    final out = await audit();
    expect(exitCode, 0, reason: out);
    expect(
      out.trim().split('\n').last,
