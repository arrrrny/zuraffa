...
void main() {
  ...
  test('A13: per-state counts, resume point, ledger totals, read-only',
      () async {
    ...
    await write('.zfa/corpus/progress.json', jsonEncode({
      'features': {
    ...
    await write('.zfa/corpus/gap-ledger.json', jsonEncode([
      {
        'id': 'gap-001',
        'kind': 'gap',
    ...
    expect(
      lastLine,
      startsWith('corpus: features=4 done=1 waived=0 stopped=1 '
  ...
  test('A14: exit 0 exactly when every manifest feature is done|waived',
      () async {
    await fx.writeManifest([
      (name: 'f1-done', ready: true, reason: ''),
