...
Future<File> _writePair(
  Behavior b,
  Directory tmp,
) async {
  final subjectPath = p.join(tmp.path, 'lib', 'tdd', '035-fixture',
      'contract_a1_subject.dart');
  final testPath = p.join(
      tmp.path, 'test', 'tdd', '035-fixture', 'contract_a1_test.dart');
...
List<String> _paramNamesOf(String subjectSource, String method) {
  ...
  while (true) {
    ...
    final linePrefix = subjectSource
        .substring(lineStart, open)
        .trim();
...
void main() {
  ...
  test('B1: the issue repro — bare-name cells keep their declared names',
      () async {
    final subject = await _writePair(
      _contractBehavior('LoginValidation.validate(email, password) -> LoginVerdict'),
      tmp,
    );
    final source = await subject.readAsString();
    final names = _paramNamesOf(source, 'validate');
    expect(names, ['email', 'password'],
        reason: 'the declared parameter names survive into the stub — '
  ...
  test('B3: unnamed generic-type params fall back to positional names',
    ...
    expect(names, ['arg0', 'arg1'],
        reason: 'the positional fallback numbers per position — '
  ...
  test('B4: the generated test summary echoes unique names', () async {
    ...
    final subjectPath = p.join(
        tmp.path, 'lib', 'tdd', '035-fixture', 'contract_a1_subject.dart');
    await const ContractTestWriter()
    ...
    expect(source, isNot(contains('arg0, Object? arg0')),
