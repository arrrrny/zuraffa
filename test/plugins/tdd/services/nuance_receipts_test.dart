...
void main() {
  ...
  test('U14: record() writes (file, reason, diff-hash) into the ledger',
      () async {
    ...
    expect(entry.file, 'lib/src/data/datasources/user/user_mock_datasource.dart');
    ...
    final ledgerFile = File(p.join(featureDir, 'tdd', 'provenance-ledger.json'));
    ...
    expect(ledger['entries'].first['reason'],
  ...
  test('U16: detect() finds drifted and unreceipted hand-deltas', () async {
    ...
    const original = 'getIt.registerLazySingleton<UserMockDataSource>(() => '
        'UserMockDataSource());';
    ...
    final unreceipted =
        await receipts().detect(files: ['lib/src/di/other.dart']);
