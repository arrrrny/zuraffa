...
import 'package:zuraffa/src/plugins/tdd/services/hand_delta_receipt.dart';
...
Future<List<ReceiptRecord>> handDeltaReceiptsOf(TddFixture fx) async =>
    ...
    ).loadAll()).where((r) => r.receipt.input['hand_delta'] == true).toList();
...
Future<String?> latestReceiptDigestOf(TddFixture fx, String relPath) async {
  String? latest;
  for (final record in await ReceiptStore(
    projectRoot: fx.root.path,
  ).loadAll()) {
...
void main() {
  ...
  group('issue #1423 — verify-red --re-certify re-receipts the hand-delta', () {
    test('E1: the drifted receipted subject is re-hashed (action update, '
        'current digest) and the hand-delta state proves clean', () async {
      ...
      expect(all.where((r) => r.receipt.command == 'tdd gen'), hasLength(1));
  ...
  group('issue #1423 — make skip re-receipts the hand-delta', () {
    test('E2: the skip transition re-hashes the drifted receipted pair; the '
        'plain red path does not (SC-1 + red-path backward compat)', () async {
      const desc = 'returns 42 when invoked with no args';
      const id = 'A2';
      const redId = 'U2';
      await fx.seedCertifiedRed(
      ...
      expect(
        report.findings.where((f) => f.kind == ProofFinding.kindModified),
        isEmpty,
        reason: 'findings: ${report.findings.map((f) => f.toJson())}',
      );
    });

    test('E4: a skip over an UNCHANGED pair appends NO hand-delta receipt — '
        'the gen receipts still validate (backward compatibility)', () async {
      const desc = 'returns 42 when invoked with no args';
      const id = 'A4';
      await fx.seedCertifiedRed(
        id: id,
...
''',
  ...
  group(
    'issue #1423 — verify is reachable for a hand-delta\u00b7d feature',
    () {
      test('E3: gen receipts -> hand edits -> re-certify -> make skip -> '
          '`zfa tdd verify` reaches the mutation audit (SC-3)', () async {
        const desc = 'returns 42 when invoked with no args';
        const id = 'A3';
        await fx.seedCertifiedRed(
          id: id,
        ...
        expect(
          ...
          reason:
              'the certified hand-delta must pass the proof preflight:\n'
              '$out',
        );
        expect(
          out,
...
/// S-tier — the `HandDeltaReceipts` service contract (fast, pure dart:io,
/// the bug #1311 B7/B8 convention). Pins the mechanics the certifying
/// transitions rely on before any CLI wiring is consulted.
void _serviceTier() {
  late Directory tmp;

  setUp(() {
    tmp = Directory.systemTemp.createTempSync('issue1423_service_');
  });
  ...
  Future<void> seedReceipt(String rel, String content) async {
    ...
    await ReceiptStore(projectRoot: tmp.path).save(
      GenerationReceipt(
        command: 'tdd gen',
        target: 'B1',
        repro: 'zfa tdd gen B1',
        at: DateTime.now().toUtc(),
        generatorVersion: 'tdd-fixture',
        input: {'feature': '1423-fixture'},
        files: [
          GenerationReceiptFile(
            path: rel,
            action: 'create',
  ...
  test('S1: drifted receipted pair -> fired, ONE appended event re-hashed '
      'from disk; ProofChecker reports zero modified findings after', () async {
    await seedFile('test/b1_test.dart', 'test v1\n');
    await seedFile('lib/b1_subject.dart', 'subject v1\n');
    await seedReceipt('test/b1_test.dart', 'test v1\n');
    await seedReceipt('lib/b1_subject.dart', 'subject v1\n');

    // The hand-delta: both receipted artifacts drift.
    await seedFile('test/b1_test.dart', 'test v2 — hand-written\n');
    await seedFile('lib/b1_subject.dart', 'subject v2 — implemented\n');

    final report = await HandDeltaReceipts.refresh(
    ...
    expect(report.fired, isTrue);
    expect(report.receiptedPaths, ['lib/b1_subject.dart', 'test/b1_test.dart']);
    ...
    expect(event.input['hand_delta'], true);
    expect(event.input['transition'], 're-certify');
    expect(event.input['behavior'], 'B1');
    expect(event.files, hasLength(2));
    for (final entry in event.files) {
      expect(entry.action, 'update');
      expect(
        entry.sha256,
        crypto.sha256
            .convert(await File(p.join(tmp.path, entry.path)).readAsBytes())
            .toString(),
        reason: 'digest re-derived from the CURRENT disk bytes',
      );
    }

    // Append-only: the gen receipts stay alongside the event.
    expect(records.where((r) => r.receipt.command == 'tdd gen'), hasLength(2));
    final checker = await ProofChecker(projectRoot: tmp.path).check();
    expect(
      checker.findings.where((f) => f.kind == ProofFinding.kindModified),
      isEmpty,
      reason:
          'latest-wins must resolve to the certified hand-delta: '
          '${checker.findings.map((f) => f.toJson())}',
    );
  });

  test('S2: no drift -> fired=false, NO receipt document appended', () async {
    await seedFile('test/b1_test.dart', 'test v1\n');
    await seedReceipt('test/b1_test.dart', 'test v1\n');
    final before = await ReceiptStore(projectRoot: tmp.path).loadAll();
    ...
    final report = await HandDeltaReceipts.refresh(
      ...
      command: 'tdd make',
      transition: 'skip',
      artifactPaths: const ['test/b1_test.dart'],
    );

    expect(report.fired, isFalse);
    expect(report.receiptedPaths, isEmpty);
    final after = await ReceiptStore(projectRoot: tmp.path).loadAll();
    expect(after, hasLength(before.length), reason: 'idempotent no-op');
    expect(after.where((r) => r.receipt.input['hand_delta'] == true), isEmpty);
  });

  test('S3: unreceipted artifact paths -> fired=false (provenance is never '
      'fabricated)', () async {
    await seedFile('lib/unreceipted.dart', 'no receipt covers me\n');
    await seedFile('test/b1_test.dart', 'test v1\n');
    await seedReceipt('test/b1_test.dart', 'test v1\n');
    ...
    final report = await HandDeltaReceipts.refresh(
      ...
      command: 'tdd make',
      transition: 'skip',
      artifactPaths: const ['lib/unreceipted.dart', 'test/b1_test.dart'],
    );

    expect(report.fired, isFalse, reason: 'only the unreceipted path drifted');
    final records = await ReceiptStore(projectRoot: tmp.path).loadAll();
    expect(
      records.where((r) => r.receipt.input['hand_delta'] == true),
      isEmpty,
    );
  });

  test('S4: a vanished receipted artifact is skipped honestly — no fabricated '
      'digest, the deleted finding stays visible', () async {
    await seedFile('test/b1_test.dart', 'test v1\n');
    await seedReceipt('test/b1_test.dart', 'test v1\n');
    File(p.join(tmp.path, 'test', 'b1_test.dart')).deleteSync();
    ...
    final checker = await ProofChecker(projectRoot: tmp.path).check();
    expect(
      checker.findings.map((f) => f.kind),
      contains(ProofFinding.kindDeleted),
      reason: 'the deletion must stay visible to the checker',
    );
  });

  test('S5: legacy machine-absolute recorded forms normalize to the '
      'project-relative receipt paths (issue #1397 compat)', () async {
    await seedFile('lib/b1_subject.dart', 'subject v1\n');
    await seedReceipt('lib/b1_subject.dart', 'subject v1\n');
    await seedFile('lib/b1_subject.dart', 'subject v2 — implemented\n');

    final legacyAbsolute = p.join(tmp.path, 'lib', 'b1_subject.dart');
    final report = await HandDeltaReceipts.refresh(
      projectRoot: tmp.path,
      feature: '1423-fixture',
      behaviorId: 'B1',
      command: 'tdd make',
      transition: 'skip',
      artifactPaths: [legacyAbsolute],
    );

    expect(report.fired, isTrue, reason: 'the absolute form must resolve');
    expect(report.receiptedPaths, ['lib/b1_subject.dart']);
    final checker = await ProofChecker(projectRoot: tmp.path).check();
    expect(
      checker.findings.where((f) => f.kind == ProofFinding.kindModified),
      isEmpty,
