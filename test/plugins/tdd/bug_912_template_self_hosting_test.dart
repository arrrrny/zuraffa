...
void main() {
  group('Template self-hosting defects (issue #912)', () {
    test(
      'A1: BehaviorTestWriter escapes apostrophes in persistence test descriptions',
      () async {
        ...
        expect(
          content,
          contains("test('U1 - persist the user\\'s theme preference'"),
        );
    ...
    test(
      'A2: migrate-paths rewrites package-URI imports in moved tests',
      () async {
        final fx = await TddFixture.create(featureName: '001-composed-feature');

        final flatTest = File(
          p.join(fx.root.path, 'test', 'tdd', 'b1_test.dart'),
        );
...
''');
        ...
        final flatSubject = File(
          p.join(fx.root.path, 'lib', 'tdd', 'b1_subject.dart'),
        );
        await flatSubject.parent.create(recursive: true);
        await flatSubject.writeAsString('// subject\n');

        final artifactsFile = File(
          p.join(
            fx.root.path,
            'specs',
            '001-composed-feature',
            'tdd',
            'artifacts.json',
          ),
        );
        ...
        expect(
          migratedContent,
          contains(
            'package:tdd_fixture/tdd/001-composed-feature/b1_subject.dart',
          ),
        );
      },
    );
