...
void main() {
  ...
  Future<String> readTestList() =>
      File(p.join(featureDir, 'tdd', 'test-list.md')).readAsString();
  ...
  test('A1: table-declared Key Entities land in the plan artifact with fields '
      'and purpose', () async {
...
''');
  ...
  test('A2: legacy bullet-format Key Entities section still extracts (the '
      'Template Version marker keeps legacy specs planning)', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A3: entities planned from a table are read back by '
      'TestListReader.readEntities (phase-0 seam)', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
    ...
    expect(
      entities.first.fields,
      containsAll(<String>['id:String', 'sizeEu:double']),
    );
  });
...
''', versionMarker: '**Template Version**: `zuraffa-2.0`');
  ...
  test('A6: Template Version zuraffa-1.0 -> plan succeeds (exit 0, artifact '
      'written)', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A7: External Dependencies & Contracts table lands row-for-row in the '
      'plan artifact (dependency, type, contract, mock priority)', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A9: a requirement referencing an undeclared external (Hive) -> exit 2 '
      'naming the dependency + fix line + no artifacts', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A10: a spec with no Key Entities section plans cleanly (no entity '
      'section rendered, exit 0)', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A11: a Key Entities section mixing a table and legacy bullets extracts '
      'both forms', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A13: a spec with BOTH a missing Template Version AND a coverage gap '
      'exits 3 (version gate runs before the coverage gate)', () async {
    await writeRawSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A14: dependencies and layer contracts land in the plan artifact in a '
      'shape TestListReader can parse (readDependencies + readLayerContracts '
      'read them back)', () async {
    await writeSpec(featureDir, '''
# Spec: 001-demo
...
''');
  ...
  test('A15: a Template Version marker that appears only inside a fenced code '
      'block (e.g. a "How to write a spec" example) is NOT treated as the '
      "spec's treaty pin — pin absent => exit 3, not exit 0", () async {
