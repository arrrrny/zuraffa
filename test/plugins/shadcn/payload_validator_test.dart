...
void main() {
  ...
  Map<String, dynamic> payload(Map<String, dynamic> tree,
  ...
  group('UiPayloadValidator', () {
    test('valid payload passes (US-3 scenario 1)', () {
      final result = validator.validate(payload(node('card', children: [
    ...
    test('unknown node identified by name and position (US-3 scenario 2)',
      ...
      final error = result.errors
          .firstWhere((e) => e.kind == ValidationErrorCategory.unknownNode);
    ...
    test('bad token reports offender and allowed set', () {
      ...
      expect(result.valid, false);
      final error = result.errors
          .firstWhere((e) => e.kind == ValidationErrorCategory.badToken);
      expect(error.message, contains('sparkly'));
      expect(error.message, contains('primary'));
    });

    test('raw color detected in styleToken with token suggestion (US-3 scenario 4)',
        () {
      final result = validator.validate(
      ...
      expect(result.valid, false);
      final error = result.errors
          .firstWhere((e) => e.kind == ValidationErrorCategory.rawColor);
    ...
    test('raw color detected in prop values', () {
      ...
      expect(
        result.errors
            .any((e) => e.kind == ValidationErrorCategory.rawColor),
        true,
      );
    });
    ...
    test('depth cap pinpoints the deep path', () {
      ...
      expect(result.valid, false);
      final error = result.errors
          .firstWhere((e) => e.kind == ValidationErrorCategory.depthCap);
    ...
    test('count cap reports actual vs cap', () {
      ...
      final result = validator.validate(payload(node('list', children: children)));
      expect(result.valid, false);
      final error = result.errors
          .firstWhere((e) => e.kind == ValidationErrorCategory.countCap);
      expect(error.message, contains('301')); // 300 children + the list node
    });

    test('invalid action id reports the grammar (US-3 scenario 5)', () {
      final result = validator.validate(payload(node('button', props: {
        'label': 'Go',
      }, actionId: 'Select Offer!')));
      expect(result.valid, false);
      final error = result.errors
          .firstWhere((e) => e.kind == ValidationErrorCategory.invalidAction);
    ...
    test('invalid nesting pinpoints parent/child and the constraint', () {
      final result = validator.validate(payload(node('select', children: [
        node('card'),
      ])));
      expect(result.valid, false);
      final error = result.errors
          .firstWhere((e) => e.kind == ValidationErrorCategory.invalidNesting);
    ...
    test('children count cap (per-node max) is a nesting violation', () {
      ...
      final result = validator.validate(payload(node('text', children: [
        node('text'),
      ])));
      expect(result.valid, false);
      expect(
        result.errors.any((e) => e.kind == ValidationErrorCategory.invalidNesting),
        true,
      );
    });

    test('all violations reported in one pass (SC-003, zero false negatives)',
        () {
      final result = validator.validate(payload(node('card', children: [
        node('ghost_node'),
        node('text', styleToken: 'neon', children: [
          node('button', actionId: 'BAD ACTION'),
        ]),
      ])));
      expect(result.valid, false);
      expect(
        result.errors.any((e) => e.kind == ValidationErrorCategory.unknownNode),
        true,
      );
      expect(
      ...
      expect(
        result.errors.any((e) => e.kind == ValidationErrorCategory.invalidAction),
        true,
      );
    });
    ...
    test('invalid JSON surfaces a parse error with path (Edge Cases)', () {
      ...
      expect(
        ...
        throwsA(isA<UiPayloadParseException>().having(
          (e) => e.message,
          'message',
    ...
    test('pin mismatch is a warning, not an error (US-5 scenario 3)', () {
      final result = validator.validate(
        payload(node('text', props: {'value': 'x'}),
            schemaVersion: '0.9.0'),
      );
    ...
    test('payload with composite from a registry validates', () {
      ...
      File('${dir.path}/offer_card.json').writeAsStringSync(jsonEncode({
        'name': 'offer_card',
        'category': 'composite',
        'props': {
          'title': {'type': 'string'},
      ...
      final result = compositeValidator.validate(payload(node('offer_card',
          props: {'title': 'Sale'},
          children: [node('text', props: {'value': 'y'})])));
      expect(result.valid, true, reason: '${result.errors}');
    });
  });
}
