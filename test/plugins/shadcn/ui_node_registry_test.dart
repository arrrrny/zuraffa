...
void main() {
  ...
  group('NodeRegistry built-ins', () {
    test('covers the full built-in component set', () {
      ...
      const expected = {
        'root', 'container', 'card', 'text', 'heading', 'button', 'input',
        'textarea', 'checkbox', 'switch', 'select', 'option', 'image',
        'avatar', 'badge', 'list', 'row', 'column', 'divider', 'progress',
        'slider', 'tooltip', 'alert', 'label',
      };
      for (final name in expected) {
        expect(registry.contains(name), true,
  ...
  group('NodeRegistry composite loading', () {
    test('composites load from .zfa/ui/components/*.json as first-class entries',
        () {
      ...
      expect(registry.definition('offer_card')!.props['title']!.type, 'string');
    ...
    test('malformed registration surfaces a clear error', () {
      ...
      expect(
        ...
        throwsA(isA<NodeRegistryException>().having(
          (e) => e.message,
          'message',
