...
void main() {
  ...
  group('VocabularySchemaExporter', () {
    test('export shape: schemaVersion + components + rules + tokens + grammar',
        () {
      ...
      expect(
        RegExp(r'^\d+\.\d+\.\d+$').hasMatch(export['schemaVersion'] as String),
        true,
    ...
    test('composites appear alongside built-ins in the export (US-2 scenario 2)',
        () {
  ...
  group('UiRenderInputSchema (SC-004)', () {
    test('ui.render tree input schema derives from the export as-is', () {
      ...
      final typeEnum = ((nodeSchema['properties']
          as Map<String, dynamic>)['type'] as Map<String, dynamic>)['enum']
          as List<dynamic>;
      expect(typeEnum, contains('card'));
      expect(typeEnum, contains('button'));
      expect(typeEnum.length,
          (export['components'] as Map).length);

      // Style token enum and action grammar flow through unchanged.
      final styleEnum = ((nodeSchema['properties']
              as Map<String, dynamic>)['styleToken']
          as Map<String, dynamic>)['enum'] as List<dynamic>;
      expect(styleEnum, contains('primary'));
      final actionPattern = ((nodeSchema['properties']
              as Map<String, dynamic>)['actionId']
          as Map<String, dynamic>)['pattern'];
      expect(actionPattern,
          (export['actionIdGrammar'] as Map)['pattern']);
