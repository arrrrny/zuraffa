...
void main() {
  group('RouteDriftDetector', () {
    ...
    test('U2.5: finding sources use the canonical total ordering', () {
      const table = RouteTable(
        version: 1,
        routes: [
          RouteEntry(
      ...
      final sources = const RouteDriftDetector().detect(table).single.sources;
      expect(sources.map((entry) => entry.name), ['Earlier', 'Later', 'Dda']);
    });
  });
}
