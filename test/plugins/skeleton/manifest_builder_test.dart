...
void main() {
  ...
  group('ManifestBuilder.render', () {
    test(
      ...
      () {
        final yaml = builder.render(makeManifest(entities: ['Cart', 'CartItem']));
    ...
    test(
      ...
      () {
        final manifest = makeManifest(
          dependencies: [
            const BoneDependency(bone: 'product-catalog', entities: ['Product']),
          ],
        );
    ...
    test(
      ...
      () {
        final manifest = makeManifest(
          specVersion: 'sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb',
        );
        final yaml = builder.render(manifest);
        final parsed = loadYaml(yaml) as Map;
    ...
    test(
      ...
      () {
        ...
        expect(parsed.containsKey('xray'), isTrue, reason: 'xray key must be present');
        final xray = parsed['xray'] as Map;
        expect(xray['overlay'], equals('{"enabled": true, "color": "neon-green"}'));
        expect(xray['mode'], equals('development'));
      },
    );

    test(
      'U33: manifest without xray markers omits the xray key',
      () {
        final yaml = builder.render(makeManifest());
        final parsed = loadYaml(yaml) as Map;

        expect(parsed.containsKey('xray'), isFalse,
