...
void main() {
  group('SC-006: pure-Dart init + graceful degradation', () {
    test(
      'A25: the plugin compiles under dart (not flutter) — types are '
    ...
    test(
      'A25 (continued): pubspec.yaml does not declare a flutter SDK '
    ...
    test(
      'A15 / SC-006: non-TTY stdout → TtyGuard.requireTty throws with a '
      'clear message',
      () {
        ...
        if (!guard.isTty()) {
          expect(
            guard.requireTty,
            throwsA(isA<TuiNonTtyException>()),
          );
        } else {
    ...
    test(
      'A19 / SC-006: engine-init failure → TuiEngineInitException with an '
      'actionable message',
      () {
