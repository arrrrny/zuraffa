...
void main() {
  group('Navigator + FocusScope widgets (FR-004, FR-006)', () {
    ...
    test(
      'A7 / U18: Navigator push/pop is exposed via the Zuraffa widget barrel and renders the home route',
      () async {
    ...
    test(
      'U19: FocusScope renders its children (cycle behavior verified by structure)',
      () async {
        final tester = await nocterm.NoctermTester.create(
          size: const nocterm.Size(40, 6),
        );
          ...
          const nocterm.FocusScope(
            ...
            child: nocterm.Center(
              child: nocterm.Column(
                children: [nocterm.Text('field-a'), nocterm.Text('field-b')],
              ),
            ),
          ),
        );
    ...
    test(
      'U15: TextInput (TextField) is exposed via the Zuraffa widget barrel and accepts a controller',
      () async {
        final tester = await nocterm.NoctermTester.create(
          size: const nocterm.Size(40, 5),
        );
        addTearDown(tester.dispose);
          ...
          nocterm.Center(
            child: nocterm.Column(
              children: [
                ...
                nocterm.TextField(controller: controller, focused: true),
              ],
            ),
          ),
        );
