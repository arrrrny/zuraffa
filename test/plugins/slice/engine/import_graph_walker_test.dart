...
void main() {
  ...
  group('ImportGraphWalker closure (FR-001)', () {
    test(
      'U20: closure from one entry is exactly the reachable local files',
      () async {
    ...
    test(
      'U20: the closure contains each reachable file exactly once',
      () async {
...
''');
    ...
    test(
      'U22: a missing entry reports the attempted path and alternatives',
      () async {
        expect(
    ...
    test('U23: view depth excludes presenter, usecases, and domain', () async {
  ...
  group('FileGraph', () {
    test('getTransitiveClosure walks imports transitively', () async {
      ...
      final viewPath = graph.nodes.keys.singleWhere(
        (p) => p.endsWith('product_view.dart'),
      );
