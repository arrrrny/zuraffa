  ...
  group('slice cut acceptance (US1)', () {
    test(
      ...
      () async {
        ...
        expect(
          files,
          contains('lib/src/domain/entities/product/product.g.dart'),
        );
        expect(
          files,
        ...
        expect(
          files,
          isNot(contains('lib/src/presentation/widgets/secondary_button.dart')),
        );
        expect(
          files,
          isNot(
            contains('lib/src/presentation/widgets/loading_indicator.dart'),
          ),
        );
        expect(
          files,
          isNot(contains('lib/src/presentation/widgets/app_card.dart')),
        );
    ...
    test('A2 (T087/T088): shared widget included and classified shared in '
      ...
      expect(
        byPath['lib/src/presentation/widgets/primary_button.dart'],
        equals('shared'),
      );
      expect(
    ...
    test('A3 (T089): getIt-resolved usecases and their DI files are '
      ...
      expect(
        files,
        contains('lib/src/di/usecases/update_product_usecase_di.dart'),
      );
      expect(
        files,
        contains('lib/src/di/usecases/fetch_settings_usecase_di.dart'),
      );

      // The generated slice_di delegates real wiring to those files and
      // registers the boundary mock.
      final sliceDi = File(
        '${sandbox()}/lib/src/di/slice_di.dart',
      ).readAsStringSync();
    ...
    test(
      'A4 (T090): a barrel import pulls only the referenced symbols',
      () async {
        await captureOutput(
          () => runner.run([
            'slice',
...
''');
    ...
    test(
      'A1: slice.yaml records hashes and boundaries at feature depth',
      () async {
        await captureOutput(
          () => runner.run([
            'slice',
        ...
        final viewEntry = filesNode.whereType<Map>().singleWhere(
          (f) => f['path'].toString().contains('product_view.dart'),
        );
        ...
        final repoBoundary = boundaries.whereType<Map>().singleWhere(
    ...
    test(
      'cutting into an existing slice name fails without clobbering',
      () async {
        await captureOutput(
          () => runner.run([
            'slice',
    ...
    test(
      'a missing entry fails with alternatives, never a stack trace',
      () async {
        final output = await captureOutput(
          () => runner.run([
            'slice',
