...
void main() {
  ...
  group('AppShellBuilder.buildMain — xray wiring', () {
    ...
    test('emits platform-safe bridge imports when xray is true', () {
      ...
      // The default (non-io) target is the generated launcher stub so the
      // conditional import always has a compilable web implementation.
      expect(src, contains("import 'xray_bridge_launcher_stub.dart'"));
    });

    test('does NOT emit bool.fromEnvironment guard when xray is true', () {
      final src = builder.buildMain(appName: 'my_app', xray: true);
      // dart.library.io is a conditional-import URI scheme, not a
      // compile-time environment variable — the conditional import above
      // already resolves per platform, so the guard is redundant.
      expect(src, isNot(contains('bool.fromEnvironment')));
    });
  ...
  group('AppShellBuilder.buildXRayBridgeLauncherStub', () {
    test('emits the XRayBridgeServer class', () {
      final src = builder.buildXRayBridgeLauncherStub();
      expect(src, contains('class XRayBridgeServer'));
    });

    test('emits the no-op start() sentinel', () {
      final src = builder.buildXRayBridgeLauncherStub();
      expect(src, contains('Future<int> start()'));
      expect(src, contains('=> -1'));
    });

    test('does NOT import dart:io (web-safe)', () {
      final src = builder.buildXRayBridgeLauncherStub();
      // A dart:io import would break web compilation — the stub must only
      // use dart:core / dart:async APIs.
