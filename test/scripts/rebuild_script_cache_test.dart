...
void main() {
  test('rebuild.sh fully invalidates build + .dart_tool caches', () {
    ...
    expect(content, contains('rm -rf build .dart_tool'),
        reason: 'rebuild.sh must fully clear build + .dart_tool so the installed '
    ...
    expect(content, isNot(contains('rm -rf .dart_tool/build_cache')),
        reason: 'Partial .dart_tool/build_cache cleanup alone caused stale '
            'binaries; use the full `rm -rf build .dart_tool` clean.');
    expect(content, isNot(contains('rm -rf .dart_tool/hooks_runner')),
        reason: 'Partial .dart_tool/hooks_runner cleanup alone caused stale '
            'binaries; use the full `rm -rf build .dart_tool` clean.');
  });
}
