...
void main() {
  ...
  group('Issue #409: NoParams provider override', () {
    test('provider from existing service with NoParams includes NoParams param', () async {
...
''');
      ...
      expect(
        ...
        isTrue,
        reason: 'Provider method should include NoParams params parameter to match service signature',
      );
    });

    test('provider with explicit NoParams config includes NoParams param', () async {
      final plugin = ProviderPlugin(
        outputDir: outputDir,
        options: const GeneratorOptions(dryRun: false, force: true),
      );
      ...
      expect(
        ...
        isTrue,
        reason: 'Provider method with NoParams config should still include the parameter',
      );
    });
  });
}
