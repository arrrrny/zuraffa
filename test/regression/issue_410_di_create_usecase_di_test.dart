...
void main() {
  ...
  group('issue #410 — zfa di create <Entity>', () {
    test('entity-based default emits per-method usecase DI (get/update) '
        'referencing real GetXUseCase / UpdateXUseCase classes', () async {
      ...
      final getDi = File(
        '$outputDir/di/usecases/get_my_entity_usecase_di.dart',
      );
      final updateDi = File(
        '$outputDir/di/usecases/update_my_entity_usecase_di.dart',
      );
      expect(
        getDi.existsSync(),
        isTrue,
      ...
      expect(
        ...
        isFalse,
        reason:
            'my_entity_usecase_di.dart referencing non-existent '
            'MyEntityUseCase must NOT be generated for entity-based flows',
      );
      expect(
        files.any((p) => p.split('/').last == 'my_entity_usecase_di.dart'),
        isFalse,
        reason:
            'the broken unified my_entity_usecase_di.dart must not '
      ...
      expect(getContent.contains('() => GetMyEntityUseCase('), isTrue);
      expect(
        updateContent.contains('registerLazySingleton<UpdateMyEntityUseCase>'),
        isTrue,
      );
      expect(updateContent.contains('() => UpdateMyEntityUseCase('), isTrue);
      ...
      expect(
        ...
        isFalse,
        reason:
            'get_my_entity_usecase_di.dart must not reference the '
      ...
      expect(
        ...
        isFalse,
        reason:
            'update_my_entity_usecase_di.dart must not reference the '
            'non-existent unified MyEntityUseCase type',
      );
    });

    test('DI imports resolve to the actual usecase files emitted by '
        '`zfa usecase create <Entity>`', () async {
      ...
      for (final diFile in diUsecaseFiles) {
        ...
        for (final match in importPattern.allMatches(content)) {
          ...
          expect(
            ...
            isTrue,
            reason:
                'DI import "$importLine" in ${diFile.path} must resolve '
    ...
    test('explicit --methods overrides the default', () async {
      ...
      final getDi = File('$outputDir/di/usecases/get_document_usecase_di.dart');
      final createDi = File(
    ...
    test('custom-usecase path (--no-entity) still emits the unified '
        '<name>_usecase_di.dart referencing <Name>UseCase', () async {
      final diPlugin = DiPlugin(
        outputDir: outputDir,
        options: const GeneratorOptions(),
      );
    ...
    test('custom-usecase path (--repo) still emits unified DI '
        '(preserved backward-compat)', () async {
      final diPlugin = DiPlugin(
        outputDir: outputDir,
        options: const GeneratorOptions(),
      );
      ...
      final unifiedDi = File('$outputDir/di/usecases/profile_usecase_di.dart');
