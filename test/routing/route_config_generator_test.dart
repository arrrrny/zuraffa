...
void main() {
  group('RouteConfigGenerator', () {
    test('flat route renders GoRoute with params binding + view return',
      ...
      expect(config.routerSource,
          contains('ProductViewRouteParams.fromMaps('));
      expect(config.routerSource, contains('return const ProductView();'));
      expect(config.routerSource,
          contains("import 'package:go_router/go_router.dart';"));
      expect(
          config.routerSource, contains("import 'package:zuraffa/zuraffa.dart';"));
      expect(
        config.routerSource,
        contains(
            "import 'package:test_app/src/views/productview.dart';"),
      );
    });
    ...
    test('typed RouteParams class renders with typed fields + fromMaps', () {
      final config = RouteConfigGenerator.generate(
        packageName: 'test_app',
        routes: [
          _decl('ProductView', '/products/:id',
              params: {'id': 'int'}, query: ['tab']),
        ],
        redirects: const [],
        guardIndex: const {},
      );
      ...
      expect(
        config.routerSource,
        contains(
            "ZfaRouteParams.intParam(pathParameters, 'id')"),
      );
      expect(
        config.routerSource,
    ...
    test('shell route nests children with relative paths', () {
      final config = RouteConfigGenerator.generate(
        packageName: 'test_app',
        routes: [
          _decl('DashboardView', '/dashboard',
              isShell: true, viewAcceptsChild: true),
      ...
      final routesStart =
          config.routerSource.indexOf('routes: [', shellStart);
      final routesEnd = config.routerSource.indexOf('],', routesStart);
      final shellBody =
          config.routerSource.substring(routesStart, routesEnd);
    ...
    test('shell view without child param is rendered without child arg', () {
      ...
      expect(
          config.routerSource, isNot(contains('PlainView(child: child)')));
    });
    ...
    test('redirect rules render GoRoute redirect entries', () {
      final config = RouteConfigGenerator.generate(
        ...
        redirects: [
          RouteRedirectRule(
              from: '/old-page', to: '/new-page', filePath: 'f', line: 1),
        ],
        guardIndex: const {},
      );
      expect(config.routerSource,
    ...
    test('guard middleware wraps route with zfaGuardRedirect', () {
      final config = RouteConfigGenerator.generate(
        ...
        redirects: const [],
        guardIndex: {
          'AuthGuard': 'src/guards/auth_guard.dart',
        },
      );
    ...
    test('guardRedirect option overrides default deny target in helper', () {
      final config = RouteConfigGenerator.generate(
        packageName: 'test_app',
        routes: [
          _decl('SecureView', '/secure',
              middleware: ['AuthGuard'], guardRedirect: '/sign-in'),
        ],
        redirects: const [],
    ...
    test('generated router file parses back with zero syntax errors', () {
      final config = RouteConfigGenerator.generate(
        ...
        redirects: [
          RouteRedirectRule(
              from: '/old', to: '/products/:id', filePath: 'f', line: 1),
        ],
        guardIndex: {'AuthGuard': 'src/guards/auth_guard.dart'},
      );
    ...
    test('deep link side files include deepLinkAware paths only', () {
      ...
      expect(
          config.deepLinkFiles.keys, contains('.well-known/apple-app-site-association'));
      expect(config.deepLinkFiles.keys,
          contains('.well-known/assetlinks.json'));
      final apple = config.deepLinkFiles['.well-known/apple-app-site-association']!;
    ...
    test('deterministic ordering: same input -> identical output', () {
      ...
      final a = RouteConfigGenerator.generate(
          packageName: 'pkg', routes: routes, redirects: const [], guardIndex: const {});
    ...
    test('empty configuration renders a valid empty router', () {
      ...
      expect(config.routerSource, contains('final List<RouteBase> zfaRoutes = [];'));
      expect(config.routerSource,
