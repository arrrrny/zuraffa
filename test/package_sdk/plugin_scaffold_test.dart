...
import 'package:zuraffa/src/package/plugin_family_names.dart';
...
void main() {
  ...
  Future<PluginScaffoldResult> scaffold({
    ...
    Set<PluginPlatform> platforms = const {
    ...
    return PluginScaffold().scaffold(
  ...
  /// Every dependency name declared by [packageName] — regular,
  /// dev, AND overrides (overrides are checked separately in B5).
  ...
  group('PluginScaffold — B1 full-family layout (FR-001 / FR-011 / SC-1)', () {
    test('B1: five package dirs, per-package file set, root docs + scripts',
        () async {
      final result = await scaffold();
      final monorepo = result.monorepoPath;
      ...
      // Exactly five packages in packages/.
      final packagesDir = Directory(p.join(monorepo, 'packages'));
      expect(
        packagesDir.listSync().whereType<Directory>().map((d) => p.basename(d.path))..sort(),
        family,
      );
      ...
      for (final pkg in family) {
        ...
        for (final rel in const [
          ...
          expect(
            FileSystemEntity.existsSync(p.join(pkgDir, rel)),
            isTrue,
      ...
      for (final rel in const [
        ...
        expect(
          FileSystemEntity.existsSync(p.join(monorepo, rel)),
          isTrue,
  ...
  group('PluginScaffold — B2 dependency-graph wiring (FR-004 / SC-1)', () {
    test('B2: app→zuraffa only; core→app; adapters→app+core; nobody→adapter',
        () async {
      final result = await scaffold();
      final monorepo = result.monorepoPath;

      const adapters = ['my_plugin_android', 'my_plugin_ios', 'my_plugin_macos'];
      ...
      for (final pkg in ['my_plugin', 'my_plugin_platform', ...adapters]) {
        expect(pubspecOf(monorepo, pkg)['version'], '1.0.0',
            reason: '$pkg must start at 1.0.0');
      }

      // App: zuraffa hosted, no in-family deps.
      expect((app['dependencies'] as YamlMap)['zuraffa'], isA<String>()
          .having((v) => v.toString(), 'hosted constraint', startsWith('^')));
      expect(
      ...
      // Core: app only.
      expect((core['dependencies'] as YamlMap)['my_plugin'], '^1.0.0');
      expect(
        declaredDeps(core).where((d) => d.startsWith('my_plugin_') && d != 'my_plugin'),
        isEmpty,
        reason: 'the platform core must not depend on adapters',
      );

      // Adapters: app + core, in-family constraints ^1.0.0.
      for (final adapter in adapters) {
        final deps = pubspecOf(monorepo, adapter)['dependencies'] as YamlMap;
        expect(deps['my_plugin'], '^1.0.0', reason: '$adapter → app');
        expect(deps['my_plugin_platform'], '^1.0.0', reason: '$adapter → core');
      }
  ...
  group('PluginScaffold — B3 harness integrity (FR-008)', () {
    test('B3: every generated test exercises its barrel through a fake channel',
        () async {
      final result = await scaffold();
      final monorepo = result.monorepoPath;
      ...
      for (final pkg in const [
        ...
        expect(
          ...
          isTrue,
          reason:
              '$pkg harness must exercise the surface through a fake channel',
        );
      }
    });
  });
  ...
  group('PluginScaffold — B4 publish metadata (FR-003 / SC-2)', () {
    test('B4: every package carries complete publish metadata + LICENSE + CHANGELOG',
        () async {
      final result = await scaffold(
      ...
      for (final pkg in const [
        ...
        final spec = pubspecOf(monorepo, pkg);
        expect(spec['description'], 'Typed testing plugin for the Zuraffa ecosystem.',
            reason: '$pkg description must be the stamped one');
        expect(spec['homepage'], 'https://zuraffa.com', reason: pkg);
        expect(spec['repository'], 'https://github.com/myorg/my_plugin',
            reason: '$pkg repository must use the requested slug');
        expect(spec['issue_tracker'], 'https://github.com/myorg/my_plugin/issues',
            reason: pkg);
        expect((spec['topics'] as YamlList).isNotEmpty, isTrue, reason: pkg);
        expect(spec.containsKey('publish_to'), isFalse,
            reason: '$pkg must be publishable (no publish_to: none)');

        final license =
            File(p.join(monorepo, 'packages', pkg, 'LICENSE'));
        expect(license.existsSync() && license.readAsStringSync().contains('MIT'),
            isTrue, reason: '$pkg must ship a LICENSE (pub.dev hard requirement)');
        final changelog =
            File(p.join(monorepo, 'packages', pkg, 'CHANGELOG.md'));
        expect(
          changelog.existsSync() && changelog.readAsStringSync().trim().isNotEmpty,
          isTrue,
  ...
  group('PluginScaffold — B5 overrides placement (FR-006 / FR-013)', () {
    test('B5: siblings override by path only; hosted constraints stay in dependencies',
        () async {
      final result = await scaffold();
      final monorepo = result.monorepoPath;
      ...
      expect((coreOverrides!['my_plugin'] as YamlMap)['path'], '../my_plugin');

      final adapterOverrides =
          pubspecOf(monorepo, 'my_plugin_android')['dependency_overrides']
              as YamlMap?;
      expect(adapterOverrides, isNotNull);
      expect((adapterOverrides!['my_plugin'] as YamlMap)['path'], '../my_plugin');
      expect((adapterOverrides['my_plugin_platform'] as YamlMap)['path'],
          '../my_plugin_platform');
      ...
      expect(adapterDeps['my_plugin'], '^1.0.0');
      expect(adapterDeps['my_plugin_platform'], '^1.0.0');
    });

    test('B5b: --zuraffa-path pins the framework via overrides, hosted stays',
        () async {
      ...
      for (final pkg in const [
        ...
        final spec = pubspecOf(monorepo, pkg);
        expect((spec['dependencies'] as YamlMap)['zuraffa'], isA<String>(),
            reason: '$pkg keeps the hosted zuraffa constraint');
        final overrides = spec['dependency_overrides'] as YamlMap?;
        expect(overrides, isNotNull, reason: '$pkg must override zuraffa');
        expect((overrides!['zuraffa'] as YamlMap)['path'], repoRoot,
  ...
  group('PluginScaffold — B6 publish tooling (FR-007 / SC-2)', () {
    test('B6: prepare_for_publish aligns versions/changelogs and commits; publish order app→core→adapters',
        () async {
      final result = await scaffold();
      final monorepo = result.monorepoPath;
      ...
      final publishScript =
          File(p.join(monorepo, 'scripts', 'publish.sh')).readAsStringSync();
      final order = const [
        'my_plugin',
        'my_plugin_platform',
        'my_plugin_android',
      ...
      expect(prep.exitCode, 0,
          reason: 'prepare_for_publish failed: ${prep.stderr}');
      ...
      for (final pkg in const [
        ...
        final spec = pubspecOf(monorepo, pkg);
        expect(spec['version'], version, reason: '$pkg must align to $version');
  ...
  group('PluginScaffold — B7 platform subset (FR-005)', () {
    test('B7: android,ios yields exactly app/core/android/ios', () async {
      final result = await scaffold(platforms: const {
        PluginPlatform.android,
        PluginPlatform.ios,
      });
      final monorepo = result.monorepoPath;
      ...
      expect(
        FileSystemEntity.existsSync(p.join(monorepo, 'packages', 'my_plugin_macos')),
        isFalse,
  ...
  group('PluginScaffold — B8 selection rejection (FR-005 / FR-010)', () {
    test('B8: empty platform set refused naming the supported set', () async {
      expect(
        ...
        throwsA(
          isA<PluginScaffoldException>().having(
            (e) => e.message,
            'message',
            contains('android, ios, macos'),
          ),
        ),
      );
    ...
    test('B8b: unknown platform name refused naming the supported set', () {
      ...
      expect(
        ...
        throwsA(isA<PluginScaffoldException>()),
      );
      expect(
  ...
  group('PluginScaffold — B10 validation rails (FR-010)', () {
    ...
    test('B10b: existing target refused, tree untouched', () async {
      ...
      expect(File(p.join(existing.path, 'keep.txt')).readAsStringSync(),
          'precious');
  ...
  group('PluginScaffold — B11 dry-run purity (FR-009)', () {
    test('B11: dry-run enumerates exactly the real run and writes nothing',
        () async {
      final dryDir = await Directory.systemTemp
          .createTemp('zfa_plugin_dry_');
      try {
        final dry = await PluginScaffold().scaffold(
          PluginScaffoldRequest(
            name: 'my_plugin',
            platforms: const {
              PluginPlatform.android,
        ...
        final realParent =
            await Directory.systemTemp.createTemp('zfa_plugin_real_');
        try {
          final real = await PluginScaffold().scaffold(
            PluginScaffoldRequest(
              name: 'my_plugin',
              platforms: const {
                PluginPlatform.android,
          ...
          final realRoot = real.monorepoPath;
          final realFiles = Directory(realRoot)
              .listSync(recursive: true)
              .whereType<File>()
              .map((f) => p.relative(f.path, from: realRoot))
              .toSet();
          expect(dry.createdFiles.toSet(), realFiles,
