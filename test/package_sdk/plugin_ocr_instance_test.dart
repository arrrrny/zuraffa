...
void main() {
  ...
  final ocrDescription = 'Typed OCR support for the Zuraffa ecosystem: '
  ...
  setUpAll(() async {
    ...
    expect(scaffold.exitCode, 0, reason: 'scaffold failed: ${scaffold.stdout}'
        '${scaffold.stderr}');
  ...
  group('B1 — instance layout + stamps (FR-001 / FR-002 / FR-005 / SC-1)',
    ...
    test('B1: five packages, OCR description, repo slugs, ocr topic, '
      ...
      for (final pkg in family) {
        ...
        expect(spec['description'], ocrDescription,
            reason: '$pkg description must be the stamped OCR one');
        expect(spec['repository'], 'https://github.com/arrrrny/zuraffa_ocr',
  ...
  group('B2 — wiring, name shapes, harness integrity (FR-003 / FR-002)',
    ...
    test('B2: dependency invariants hold for the OCR instance', () async {
      ...
      final appDeps =
          pubspecOf('zuraffa_ocr')['dependencies'] as YamlMap;
      expect(appDeps['zuraffa'], '^6.2.2');
      expect(
        declaredDeps(pubspecOf('zuraffa_ocr'))
      ...
      expect(
        declaredDeps(pubspecOf('zuraffa_ocr_platform'))
      ...
      for (final adapter in adapters) {
        ...
        expect(deps['zuraffa_ocr_platform'], '^0.1.0',
            reason: '$adapter → core');
      }
      ...
      for (final pkg in ['zuraffa_ocr', 'zuraffa_ocr_platform']) {
        ...
        expect(all.where(adapters.contains), isEmpty,
      ...
      for (final adapter in const [
        ...
        final prefix = adapter == 'zuraffa_ocr_android'
            ...
            : adapter == 'zuraffa_ocr_ios'
                ? 'IosOcr'
                : 'MacosOcr';
        expect(sources, contains(prefix),
            reason: '$adapter must carry the $prefix class prefix');
      }
    });

    test('B2c: every harness imports its barrel over a test double',
        () async {
      for (final pkg in family) {
        final testDir = Directory(
          p.join(monorepo, 'packages', pkg, 'test'),
        );
        final harness = testDir
            ...
            .map((f) => f.readAsStringSync())
            .join('\n');
        expect(harness, contains('package:$pkg/$pkg.dart'),
            reason: '$pkg harness must import its barrel');
        final hasDouble = harness.toLowerCase().contains('fake') ||
            RegExp(r'class _\w+', multiLine: true).hasMatch(harness);
        expect(hasDouble, isTrue,
