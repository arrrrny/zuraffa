...
void main() {
  group('UiRenderTool', () {
    ...
    test('render emits UiRenderEventRender then UiRenderEventDone', () async {
      ...
      tool = UiRenderTool(channel: channel, idGenerator: _sequentialIds);
      tool.activateMission('default');
    ...
    test('replace_view_id_replaces_existing_view', () async {
      ...
      await Future<void>.delayed(Duration.zero);
      final replaceEvents = received.whereType<UiRenderEventReplace>().toList();
    ...
    test('emitError on validation failure', () async {
      ...
      await Future<void>.delayed(Duration.zero);
      expect(received.whereType<UiRenderEventError>(), hasLength(1));
    });
    ...
    test('rapid_render_calls_last_wins', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'the earlier view itself is valid; "last wins" applies to '
            'what is currently displayed',
      );
      expect(
        tool.activeView!.viewId,
        second.viewId,
    ...
    test(
      'renderProgressive emits partial events then a final render',
      () async {
        final channel = UiEventChannel();
        tool = UiRenderTool(channel: channel, idGenerator: _sequentialIds);
        tool.activateMission('default');
    ...
    test('vocabulary narrowing applied when mission is configured', () {
      ...
      tool = UiRenderTool(narrowingConfig: config, idGenerator: _sequentialIds);
    ...
    test(
      'renderProgressive empty stream still emits Done (no host hang)',
      () async {
        final channel = UiEventChannel();
        tool = UiRenderTool(channel: channel, idGenerator: _sequentialIds);
        tool.activateMission('default');
        ...
        final views = await tool
            .renderProgressive(const Stream<UiNode>.empty())
            .toList();
        await Future<void>.delayed(Duration.zero);
    ...
    test(
      'renderProgressive drops stale view on mid-stream validation failure',
      () async {
        final channel = UiEventChannel();
        tool = UiRenderTool(channel: channel, idGenerator: _sequentialIds);
        tool.activateMission('default');
  ...
  group('UiRenderTool routeInteraction (FR-004 / FR-006)', () {
    test('semantic_action_routed_to_agent', () async {
      ...
      final view = tool.render(
        const UiNode(
          type: 'root',
          children: [UiNode(type: 'button', actionId: 'select_offer')],
        ),
      );
      ...
      expect(received.whereType<UiRenderEventInteraction>(), hasLength(1));
    });
    ...
    test(
      'routeInteraction attributes a view-less action to the active view',
      () async {
        final router = CapturingActionRouter();
        final tool = UiRenderTool(
          actionRouter: router,
        ...
        final first = tool.render(
          const UiNode(
            type: 'root',
            children: [UiNode(type: 'button', actionId: 'first')],
          ),
        );
        final second = tool.render(
          const UiNode(
            type: 'root',
            children: [UiNode(type: 'button', actionId: 'second')],
          ),
        );
        ...
        expect(
          ...
          first.viewId,
          reason:
              'action attributed to the active view, not the '
