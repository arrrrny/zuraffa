...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final servicePath = path.join(outputDir, 'domain', 'services', 'permission_service.dart');
      expect(File(servicePath).existsSync(), isTrue, reason: 'Service file should be created');
      ...
      expect(File(servicePath).existsSync(), isFalse, reason: 'Service file should be deleted on revert');
    },
  );

  test(
    'UseCasePlugin revert deletes custom usecases',
    () async {
      ...
      final usecasePath = path.join(outputDir, 'domain', 'usecases', 'profile', 'check_permission_usecase.dart');
  ...
  test(
    'Enum imports are correctly handled in Provider and Service',
    () async {
      // 1. Setup enums directory and an enum
      final enumsDir = Directory(path.join(outputDir, 'domain', 'entities', 'enums'));
      enumsDir.createSync(recursive: true);
      File(path.join(enumsDir.path, 'permission_status.dart')).writeAsStringSync('enum PermissionStatus { allowed, denied }');
      File(path.join(enumsDir.path, 'permission_type.dart')).writeAsStringSync('enum PermissionType { camera, location }');
      File(path.join(enumsDir.path, 'index.dart')).writeAsStringSync("export 'permission_status.dart';\nexport 'permission_type.dart';");
      ...
      final enumsIdxFile = File(path.join(outputDir, 'domain', 'entities', 'enums', 'index.dart'));
      ...
      final providerPath = path.join(outputDir, 'data', 'providers', 'check_permission', 'permission_provider.dart');
      final providerFile = File(providerPath);
      if (providerFile.existsSync()) {
        final providerContent = providerFile.readAsStringSync();
        expect(providerContent, contains("import '../../../domain/entities/enums/index.dart';"));
        expect(providerContent, isNot(contains("import '../../../domain/entities/permission_status/permission_status.dart';")));
      } else {
      ...
      if (serviceFile.existsSync()) {
        ...
        expect(serviceContent, contains("import '../entities/enums/index.dart';"));
      } else {
        fail('Service file was not created');
      }

      // 5. Verify Mock Data was generated for enums correctly
      final mockDataPath = path.join(outputDir, 'data', 'mock', 'permission_status_mock_data.dart');
      expect(File(mockDataPath).existsSync(), isTrue, reason: 'Mock data should be generated for enums');
      final mockDataContent = File(mockDataPath).readAsStringSync();
      expect(mockDataContent, contains("import '../../domain/entities/enums/index.dart';"));
      expect(mockDataContent, isNot(contains("import '../../domain/entities/permission_status/permission_status.dart';")));
      expect(mockDataContent, contains('PermissionStatus.values[seed % PermissionStatus.values.length]'));
    },
  );

  test(
    'Mock data is skipped for completable usecases',
    () async {
      final config = GeneratorConfig(
      ...
      final mockDataPath = path.join(outputDir, 'data', 'mock', 'void_mock_data.dart');
      expect(File(mockDataPath).existsSync(), isFalse, reason: 'Mock data should be skipped for completable/void returns');
    },
  );
}
