...
void main() {
  group('candidatePaths (pure, environment-derived)', () {
    test('c1: emits no constant machine-specific paths when the env declares none', () {
    ...
    test('c2: includes FLUTTER_ROOT/bin/dart iff FLUTTER_ROOT is set', () {
      ...
      expect(
        withoutRoot.where((c) => c.contains('FLUTTER_ROOT')),
        isEmpty,
      );
      expect(
        withoutRoot.where((c) => c.contains('/bin/dart') && !c.startsWith('/usr/local')),
        isEmpty,
    ...
    test('c4: expands ZURAFFA_TOOLCHAIN_HINTS to <dir>/dart and <dir>/bin/dart', () {
      final candidates = DartToolchainResolver.candidatePaths(
    ...
    test('c5: keeps the generic /usr/local hint, omits user entries when home empty', () {
      final candidates = DartToolchainResolver.candidatePaths(
        environment: const {},
        home: null,
      );
    ...
    test('acceptance 2 (documented-env recipe): hints=/opt/flutter yields the old tier declaratively', () {
      final candidates = DartToolchainResolver.candidatePaths(
        environment: {'ZURAFFA_TOOLCHAIN_HINTS': '/opt/flutter'},
        home: null,
      );
      expect(candidates, contains('/opt/flutter/bin/dart'));
    });
  });
  ...
  group('resolve() tiers over injected probes', () {
    ...
    test('r1: ZURAFFA_DART_BIN pin wins over every tier when the file exists', () async {
    ...
    test('r2: a missing ZURAFFA_DART_BIN pin is skipped, falls through to PATH', () async {
    ...
    test('r4: dart next to which-flutter (symlink-resolved sibling) is found', () async {
    ...
    test('r5: existing candidates resolve in candidate order when PATH misses', () async {
      env['HOME'] = '/home/dev';
    ...
    test('mcp: dart beside the flutter symlink sibling missing keeps searching', () async {
