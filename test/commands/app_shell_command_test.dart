...
void main() {
  ...
  group('CLI registration', () {
    test('CliRunner exposes the `app` command in the top-level help',
        () async {
...
''');
      ...
      final routingDir =
          Directory(p.join(workspace.path, 'lib', 'src', 'routing'))
            ..createSync(recursive: true);
...
''');
    ...
    test('writes all three glue files when run from a project root',
        () async {
      final runner = CliRunner(exitOnCompletion: false);
      ...
      final myAppFile =
          File(p.join(workspace.path, 'lib', 'src', 'app', 'my_app.dart'));
    ...
    test('--output lib/custom writes glue there and main.dart imports resolve',
        () async {
...
''');
      final routingDir =
          Directory(p.join(workspace.path, 'lib', 'custom', 'routing'))
            ..createSync(recursive: true);
      await File(p.join(routingDir.path, 'index.dart')).writeAsString('''
// Generated by zfa
import 'package:go_router/go_router.dart';
...
''');
      ...
      expect(
        File(p.join(workspace.path, 'lib', 'custom', 'app', 'my_app.dart'))
            .existsSync(),
        isTrue,
      ...
      expect(
        File(
          p.join(workspace.path, 'lib', 'custom', 'routing', 'app_router.dart'),
        ).existsSync(),
        isTrue,
      ...
      final mainSrc =
          File(p.join(workspace.path, 'lib', 'main.dart')).readAsStringSync();
      expect(
        mainSrc,
    ...
    test('rejects --output outside lib/ with an actionable error',
        () async {
      final runner = CliRunner(exitOnCompletion: false);
      final output =
          await runner.runCapturing(['app', 'shell', '--output', 'src']);
    ...
    test('does NOT add the mock hint by default', () async {
      ...
      expect(mainFile.existsSync(), isTrue);
      expect(
        mainFile.readAsStringSync().contains('Mock DI mode'),
        isFalse,
      );
    });
    ...
    test('skips an existing lib/main.dart without --force', () async {
      ...
      expect(
        File(p.join(workspace.path, 'lib', 'src', 'routing', 'app_router.dart'))
            .existsSync(),
        isTrue,
      );
    });

    test('overwrites an existing lib/main.dart when --force is passed',
        () async {
      final mainFile = File(p.join(workspace.path, 'lib', 'main.dart'));
      await mainFile.parent.create(recursive: true);
      await mainFile.writeAsString('void main() {} // user-written\n');
    ...
    test('fails with an actionable error when setupDependencies is missing',
        () async {
      // Remove the DI index.
      await File(
    ...
    test('fails with an actionable error when getAllRoutes is missing',
        () async {
    ...
    test('fails with an actionable error when pubspec.yaml is missing',
        () async {
      // Move to an empty dir.
      final empty = await Directory.systemTemp.createTemp('zfa_app_shell_empty_');
