...
Future<void> subject_u3() async {
  ...
  final login = binding.stateBindingFor('LoginPage');
  expect(login.error, StateErrorKind.toaster);
  expect(login.empty, isFalse);
  final register = binding.stateBindingFor('RegisterPage');
  expect(register.error, StateErrorKind.inline);
  expect(register.empty, isTrue);
  expect(binding.stateBindingFor('UndeclaredView').error, StateErrorKind.none);
}
