...
class ArtifactRecord {
  ...
  /// ISO-8601 UTC modification time of the zfa binary that generated the
  /// artifacts (bug #683). Null for records written before this field
  /// existed — those skip the binary-freshness check (backwards compatible).
  final String? binaryMtime;
  ...
  /// Preserves [binaryMtime] — ownership changes never alter binary state.
  ...
  /// Copy with a new binary mtime (bug #683 — set when the artifacts are
  /// regenerated by a rebuilt binary so the next gen skips the check).
  ArtifactRecord copyWithBinaryMtime(String? binaryMtime) => ArtifactRecord(
    behaviorId: behaviorId,
    feature: feature,
    sourceCriterion: sourceCriterion,
    testPath: testPath,
  ...
  factory ArtifactRecord.fromJson(Map<String, dynamic> json) {
    ...
    return ArtifactRecord(
      ...
      // Legacy records (pre-#683) carry no binary_mtime — null skips the
      // binary-freshness check.
      binaryMtime: json['binary_mtime'] is String
          ? json['binary_mtime'] as String
          : null,
    );
  }
