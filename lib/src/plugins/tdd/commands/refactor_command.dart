...
/// deleted recursively.
///
/// Two guards keep live runners safe:
///
...
///    PathNotFoundException copying the incremental dill back. Best-effort:
///    on Linux the scan reads /proc/<pid>/cmdline; elsewhere (or on any
///    error) the set is empty and only the commandStartedAt guard applies.
///
...
Future<void> clearDartTestKernelCache(
  ...
  // Best-effort byte total of a kernel cache entry (a file, or a
  // directory tree of dill files). An entry that vanishes mid-walk only
  // undercounts the reported size — the delete still runs.
  Future<int> entrySize(FileSystemEntity entity) async {
  ...
  try {
    ...
    if (await cacheDir.exists()) {
      freedBytes += await entrySize(cacheDir);
    ...
    if (await tmpDir.exists()) {
      await for (final entity in tmpDir.list()) {
        ...
        try {
          ...
          freedBytes += await entrySize(entity);
  ...
  if (cleared > 0) {
    print(
      '   cleared $cleared stale kernel dir(s), '
...
/// referenced by ANY live process's argv (Linux only — /proc/<pid>/cmdline;
/// an empty set elsewhere or on any error, degrading to the
/// commandStartedAt guard alone).
///
