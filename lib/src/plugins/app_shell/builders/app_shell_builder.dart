...
/// barrel file is emitted by [buildXRayDecksBarrel] and the web-safe
/// default target of main.dart's conditional import is emitted by
/// [buildXRayBridgeLauncherStub] so every import main.dart emits always
/// resolves.
///
...
///
/// All generated files are stamped with a `// Generated by zfa` header so
...
class AppShellBuilder {
  ...
  String buildMain({
    ...
    if (xray) {
      // Add conditional import for XRayBridgeServer. The default (non-io)
      // target is the generated lib/xray_bridge_launcher_stub.dart — the
      // ONLY implementation on web, where dart:io is unavailable. The io
      // target is the real dart:io server. [buildXRayBridgeLauncherStub]
      // is emitted by the app-shell command alongside main.dart so the
      // conditional import always has a compilable default target.
      ...
      // The conditional import already resolves XRayBridgeServer per
      // platform (stub on web, real server on io), so `start()` is safe to
      // call unconditionally — no bool.fromEnvironment guard needed.
      final bridgeLauncher = '''

/// Platform-safe X-Ray bridge launcher.
/// The conditional import above resolves `XRayBridgeServer` to the real
/// dart:io server on native platforms and to the web-safe stub on web, so
/// `start()` is safe to call on every platform.
...
''';
  ...
  /// target of the conditional import [buildMain] emits when
  /// `xray: true`.
  ///
  /// main.dart imports this file as the non-`dart.library.io` branch:
  ///
  /// ```dart
  /// import 'xray_bridge_launcher_stub.dart'
  ///     if (dart.library.io) 'package:zuraffa_flutter/.../xray_bridge_server.dart';
  /// ```
  ///
  /// A conditional import's default target must exist for the file to
  /// analyze/compile, and on web (where `dart:io` is unavailable) this
  /// stub is the *only* implementation of `XRayBridgeServer`. It therefore
  /// mirrors the API surface main.dart actually uses — the constructor
  /// used by `XRayBridgeServer()` and `Future<int> start()` — with a
  /// no-op `start()` returning `-1` (matching the real server's
  /// release-mode no-op sentinel). It must NOT import `dart:io`; it
  /// compiles on every platform.
  String buildXRayBridgeLauncherStub() {
    return '''// Generated by zfa — web-safe stub for the X-Ray bridge launcher.
// The conditional import in main.dart resolves to this file on web
// (where dart:io is unavailable) and to the real XRayBridgeServer on
// native platforms. Do not edit manually.

/// No-op stand-in for the X-Ray bridge server (see
...
class XRayBridgeServer {
  ...
  /// No-op: the bridge server cannot run on web. Returns -1, matching the
  /// real server's release-mode no-op sentinel.
  Future<int> start() async => -1;
}
''';
  }
