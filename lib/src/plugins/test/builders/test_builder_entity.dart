...
extension TestBuilderEntity on TestBuilder {
  ...
  Future<GeneratedFile> generateForMethod(
    ...
    final mockDataFile = discovery.findFileSync(
    ...
    directives.add(Directive.import(toPackageImport(dataSourceFile!.path)));
    directives.add(Directive.import(toPackageImport(mockDataSourceFile!.path)));
    directives.add(Directive.import(toPackageImport(mockDataFile!.path)));
    directives.add(Directive.import(toPackageImport(dataRepositoryFile!.path)));
        ...
        ..body = Block((b) {
          ...
          final setUpBody = Block((s) {
            s.statements.add(
              refer(
                'mockDataSource',
              ).assign(refer('${entityName}MockDataSource').call([])).statement,
            );
            s.statements.add(
            ...
            s.statements.add(
              refer(
                'useCase',
              ).assign(refer(className).call([refer('repository')])).statement,
            );
            s.statements.add(
  ...
  List<Method> _throwingDataSourceMethods(GeneratorConfig config) {
    ...
    Method make(String name, String ret, List<(String, String)> params) =>
        Method(
          (b) => b
            ..name = name
            ..returns = refer(ret)
    ...
    return [
      ...
      make('update', 'Future<$e>', [('params', 'UpdateParams<$id, $patch>')]),
  ...
  List<Code> _nativeTestCode(
    ...
    switch (method) {
      ...
      case 'watch':
        paramsExpr = refer('QueryParams<$e>').call([], {
          'filter': refer(
            'Eq',
          ).call([refer('${e}Fields').property(qf), refer('t$e').property(qf)]),
        });
        break;
      case 'getList':
      case 'watchList':
      ...
      case 'delete':
        paramsExpr = refer(
          'DeleteParams<$id>',
        ).call([], {'id': refer('t$e').property(config.idField)});
        break;
      default:
        paramsExpr = refer('t$e');
    }
    ...
    final getOrElseThunk = Method(
      (mm) => mm
        ..lambda = true
        ..body = refer('throw').call([
          refer('Exception').call([literalString('not success')]),
        ]).code,
    ).closure;
        ...
        t.statements.add(
          declareFinal('result')
              .assign(
                refer(
                  'throwingUseCase',
                ).property('call').call([paramsExpr]).awaited,
              )
              .statement,
        );
        t.statements.add(
