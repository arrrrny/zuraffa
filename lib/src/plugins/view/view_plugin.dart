...
class ViewPlugin extends FileGeneratorPlugin implements CliAwarePlugin {
  ...
  JsonSchema get configSchema => {
    ...
    'properties': {
      ...
      'skin-anchor': {
        ...
        'items': {'type': 'string'},
        'description':
            'ZfaButton anchor contract ids this view owns (issue '
            '#1112) — one debugTap<PascalAnchor>() seam per anchor',
      },
      'v6-state': {
        'type': 'boolean',
        'default': false,
  ...
  Future<List<GeneratedFile>> generateWithContext(PluginContext context) async {
    final config = GeneratorConfig(
      ...
      skinAnchors:
          ((context.data['skin-anchor'] ?? context.data['skin_anchor'])
                  as List?)
              ?.whereType<String>()
  ...
  Future<GeneratedFile?> _emitSkinKitIfNeeded(FileSystem fs) async {
  ...
  Future<List<GeneratedFile>> generate(
    ...
    if (config.generateSkin) {
      final kitFile = await _emitSkinKitIfNeeded(fs);
      if (kitFile != null) {
  ...
  Future<List<GeneratedFile>> _generateViewFile({
    ...
    final content = classBuilder.build(
      ViewClassSpec(
        ...
        skinAnchors: config.skinAnchors,
