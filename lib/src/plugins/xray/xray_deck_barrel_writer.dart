...
class XRayDeckBarrelWriter {
  ...
  BarrelUpdateResult update({
    ...
    // Convert to URI-style separators for cross-platform consistency.
    final relativeDeckPath = p
        .relative(deckFilePath, from: p.dirname(barrelPath))
        .replaceAll(r'\', '/');
    ...
    if (!content.contains(callLine)) {
      ...
      if (insertPos >= 0) {
        final afterGuard = insertPos + releaseGuard.length;
        content =
        ...
        // No release guard — find the registerAllXRayDecks function and
        // insert before its closing brace.
        final funcDeclPattern = RegExp(r'void registerAllXRayDecks\(\) \{');
        final funcMatch = funcDeclPattern.firstMatch(content);
        if (funcMatch != null) {
          // Find the matching closing brace for this function
          final funcStart = funcMatch.end;
          var braceCount = 1;
          var funcEnd = funcStart;
          while (funcEnd < content.length && braceCount > 0) {
            if (content[funcEnd] == '{') {
              braceCount++;
            } else if (content[funcEnd] == '}') {
              braceCount--;
            }
            funcEnd++;
          }
          if (braceCount == 0) {
            ...
            final closingBracePos = funcEnd - 1;
            content =
                '${content.substring(0, closingBracePos)}'
                '$callLine\n'
                '${content.substring(closingBracePos)}';
            callAdded = true;
          }
        } else {
          // Fallback to last closing brace (old behavior)
