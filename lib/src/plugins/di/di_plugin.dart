...
class DiPlugin extends FileGeneratorPlugin implements CliAwarePlugin {
  ...
  Future<List<GeneratedFile>> generateWithContext(PluginContext context) async {
    final config = GeneratorConfig(
      ...
      //
      // Spec #972 FR-5 alignment (surfaced by issue #1194's boot test):
      // the usecase plugin's default vocabulary is `['get', 'update']` —
      // toggle is generated only when explicitly requested via --methods.
      // The stale `toggle` here emitted a `toggle_<entity>_usecase_di.dart`
      // importing a usecase class that is never generated by default, so
      // the canonical fresh slice did not even compile. The DI default now
      // matches the usecase plugin exactly.
      methods:
  ...
  Future<List<GeneratedFile>> generate(
    ...
    //
    // Issue #1194 (spec 1002 ordering, mirrored from MockPlugin): when
    // the mock plugin is co-active in THIS run it already emitted the
    // simulation binding + di/simulation/index (di runs after mock —
    // runAfter includes 'mock'). Emitting them here as well would create
    // a second transaction operation for the same files and fail the
    // commit ("Transaction failed"), so di defers to the mock plugin —
    // exactly the symmetry the mock plugin applies to di/index.dart.
    // Standalone di runs (zfa di / zfa make without the mock plugin)
    // keep emitting the binding themselves.
    final mockCoActive = context?.isActive('mock') ?? false;
    if (emitSimulationBinding && !config.revert && !mockCoActive) {
