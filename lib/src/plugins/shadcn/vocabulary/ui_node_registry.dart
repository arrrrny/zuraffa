...
class NodeRegistry {
  NodeRegistry._(this._definitions, {required this.maxDepth,
      required this.maxNodes, required this.styleTokens});
  ...
  static UiNodeDefinition _readRegistration(File file) {
    ...
    if (name == null || name.isEmpty) {
      throw NodeRegistryException(
        'Composite registration ${file.path} is missing the "name" field.',
      );
    }
    ...
    return UiNodeDefinition(
      name: name,
      category: (json['category'] as String?) ?? 'composite',
      props: props,
      children: children,
      isComposite: true,
    );
  ...
  bool isComposite(String name) =>
      _definitions[name]?.isComposite ?? false;
  ...
  List<String> get compositeNames {
    final names = _definitions.values
  ...
  static Map<String, UiNodeDefinition> _buildBuiltIns() {
    ...
    return {
      ...
      'heading': def('heading', 'content', props: {
        'value': const UiPropDefinition(name: 'value', type: 'string'),
      ...
      'label': def('label', 'content', props: {
        'value': const UiPropDefinition(name: 'value', type: 'string'),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'image': def('image', 'content', props: {
      ...
      'avatar': def('avatar', 'content', props: {
        'src': const UiPropDefinition(name: 'src', type: 'string'),
        'fallback': const UiPropDefinition(
            name: 'fallback', type: 'string', defaultValue: '?'),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'badge': def('badge', 'content', props: {
        'value': const UiPropDefinition(name: 'value', type: 'string'),
      ...
      'input': def('input', 'input', props: {
        'placeholder': const UiPropDefinition(
            name: 'placeholder', type: 'string'),
        'value': const UiPropDefinition(name: 'value', type: 'string'),
        'keyboard': const UiPropDefinition(
            name: 'keyboard', type: 'enum',
            enumValues: ['text', 'number', 'email', 'password'],
            defaultValue: 'text'),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'textarea': def('textarea', 'input', props: {
        'placeholder': const UiPropDefinition(
            name: 'placeholder', type: 'string'),
        'rows': const UiPropDefinition(
            name: 'rows', type: 'number', defaultValue: 3),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'checkbox': def('checkbox', 'input', props: {
        'checked': const UiPropDefinition(
            name: 'checked', type: 'boolean', defaultValue: false),
        'label': const UiPropDefinition(name: 'label', type: 'string'),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'switch': def('switch', 'input', props: {
        'checked': const UiPropDefinition(
            name: 'checked', type: 'boolean', defaultValue: false),
        'label': const UiPropDefinition(name: 'label', type: 'string'),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'select': def('select', 'input', props: {
        'placeholder': const UiPropDefinition(
            name: 'placeholder', type: 'string'),
        'value': const UiPropDefinition(name: 'value', type: 'string'),
      }, children: const UiChildrenConstraint(
          min: 0, max: 32, allowedChildTypes: ['option'])),
      'option': def('option', 'input', props: {
        'value': const UiPropDefinition(name: 'value', type: 'string'),
        'label': const UiPropDefinition(name: 'label', type: 'string'),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'slider': def('slider', 'input', props: {
        'min': const UiPropDefinition(name: 'min', type: 'number',
            defaultValue: 0),
        'max': const UiPropDefinition(name: 'max', type: 'number',
            defaultValue: 100),
        'value': const UiPropDefinition(name: 'value', type: 'number',
            defaultValue: 0),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'progress': def('progress', 'feedback', props: {
        'value': const UiPropDefinition(name: 'value', type: 'number',
            defaultValue: 0),
      }, children: const UiChildrenConstraint(min: 0, max: 0)),
      'tooltip': def('tooltip', 'feedback', props: {
        'message': const UiPropDefinition(name: 'message', type: 'string'),
      }, children: const UiChildrenConstraint(min: 0, max: 4)),
      'alert': def('alert', 'feedback', props: {
        'title': const UiPropDefinition(name: 'title', type: 'string'),
        'message': const UiPropDefinition(name: 'message', type: 'string'),
        'tone': const UiPropDefinition(
            name: 'tone', type: 'enum',
