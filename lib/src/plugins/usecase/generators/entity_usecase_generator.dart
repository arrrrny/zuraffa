...
class EntityUseCaseGenerator {
  ...
  /// SPEC 1119: the in-memory source build shared by generation and the
  /// conformance gate — the gate drives the SAME derivation generation
  /// drives, so a grammar change can never pass a stale expectation
  /// (spec #1127 principle, applied to the usecase surface).
  String buildUsecaseSource(GeneratorConfig config, String method) {
    return _planMethod(config, method).content;
  }

  /// SPEC 1119: the prescribed per-method contract — class, base class,
  /// variant (`future` | `stream` | `void`), result type, params type,
  /// exception type — read off the SAME derivation generation drives.
  /// Returns null for methods outside the entity vocabulary.
  UsecaseMethodContract? describeMethod(GeneratorConfig config, String method) {
    final _MethodPlan plan;
    try {
      plan = _planMethod(config, method);
    } on ArgumentError {
      return null;
    }
    final variant = plan.isStream
        ? 'stream'
        : plan.isCompletable
        ? 'void'
        : 'future';
    return UsecaseMethodContract(
      method: method,
      className: plan.className,
      baseClass: plan.baseClass.symbol,
      variant: variant,
      resultType: plan.returnType.accept(DartEmitter()).toString(),
      paramsType: plan.paramsType.accept(DartEmitter()).toString(),
      // The only exception the generated body can raise directly — the
      // cancellation guard every emitted execute carries
  ...
  Future<GeneratedFile> _generateForMethod(
    ...
    final plan = _planMethod(config, method);
    return _writeOrAppend(
      config: config,
      filePath: plan.filePath,
      className: plan.className,
      executeMethodSource: plan.executeMethodSource,
      content: plan.content,
    );
  }

  /// The full derivation of one entity-method usecase — everything
  /// generation needs to write the file and the gate/explain need to
  /// describe it. Single derivation, many consumers (SPEC 1119).
  _MethodPlan _planMethod(GeneratorConfig config, String method) {
...
/// SPEC 1119: the prescribed per-method usecase contract — the shape the
/// conformance gate audits against and `--explain` describes. Derived by
/// [EntityUseCaseGenerator.describeMethod] from the SAME derivation
/// generation drives, so it cannot drift from the grammar.
class UsecaseMethodContract {
  /// The generating method name (`get`, `update`, ...).
  final String method;

  /// The prescribed class (`Get<Product>UseCase`).
  final String className;

  /// The prescribed base class: `UseCase`, `StreamUseCase` or
  /// `CompletableUseCase`.
  final String baseClass;

  /// The execution variant: `future` (`Future<T> execute`), `stream`
  /// (`Stream<T> execute`) or `void` (`Future<void> execute`, the
  /// completable shape).
  final String variant;

  /// The result type the usecase is bound to (`Product`,
  /// `List<Product>`, `void`).
  final String resultType;

  /// The params type bound to the execute signature
  /// (`QueryParams<Product>`, `DeleteParams<String>`, ...).
  final String paramsType;

  /// The exception type the generated body can raise directly —
  /// `CancelledException`, via the cancellation guard every emitted
  /// execute carries.
  final String exception;

  /// The conventional file name (`get_product_usecase.dart`).
  final String fileName;

  /// The conventional absolute file path.
  final String filePath;

  const UsecaseMethodContract({
  ...
  /// The canonical execute signature the gate audits against.
  String get executeSignature =>
      '${variant == 'stream' ? 'Stream<$resultType>' : 'Future<$resultType>'} '
      'execute($paramsType params, CancelToken? cancelToken)';
...
/// The internal derivation plan for one entity-method usecase: the
/// resolved code_builder specs plus everything the write/append path and
/// the contract reader need.
class _MethodPlan {
  final String method;
  final String className;
  final TypeReference baseClass;
  final Reference paramsType;
  final Reference returnType;
  final bool isStream;
  final bool isCompletable;
  final String fileName;
  final String filePath;
  final String executeMethodSource;
  ...
  const _MethodPlan({
    ...
    required this.fileName,
    required this.filePath,
    required this.executeMethodSource,
