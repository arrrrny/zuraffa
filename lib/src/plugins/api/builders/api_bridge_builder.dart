...
class ApiBridgeBuilder {
  ...
  String _generateBridgeFile({
    ...
    for (final uc in useCases) {
      ...
      lines.add("import '../../$usecaseImportBase/$fileName.dart';");
    }
    lines.add('');

    // Entity import (for return types that have toJson)
    lines.add("import '../../domain/entities/$entitySnake/$entitySnake.dart';");
    ...
    lines.add('final getIt = GetIt.instance;');
    lines.add('');
    ...
    for (final uc in useCases) {
      ...
      lines.add('  ZuraffaApiBridge.registerEndpoint(');
      lines.add('    endpoint: const ApiEndpoint(');
      lines.add("      method: '$method',");
      lines.add("      domain: '$domain',");
      lines.add("      usecase: '${uc.methodName}',");
      lines.add('      params: $paramsMap,');
      lines.add("      returns: '${uc.returnType}',");
      lines.add('      isStream: ${uc.isStream},');
      lines.add('    ),');
      lines.add('    handler: _handle${_capitalize(uc.methodName)},');
      lines.add('  );');
    }
    ...
    // Handler functions
    for (final uc in useCases) {
      lines.add(_generateHandler(uc, domain));
    }
