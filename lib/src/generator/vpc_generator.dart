...
''';
  ...
  Future<GeneratedFile> generateView() async {
    ...
    // Determine if view needs route parameters
    ...
    final needsQueryParam =
        (hasGet || hasWatch) && !needsIdParam && config.queryField != 'id';

    // Add route parameter fields
    final routeParamFields = <String>[];
    final routeParamConstructor = <String>[];
    final routeParamComments = <String>[];

    if (needsIdParam) {
      routeParamFields.add('  final String? id;');
      routeParamConstructor.add('this.id,');
      routeParamComments.add(
        '/// When navigating to detail/update routes, pass the ${config.idField}',
      );
    }

    if (needsQueryParam) {
      routeParamFields.add('  final String? ${config.queryField};');
      routeParamConstructor.add('this.${config.queryField},');
      routeParamComments.add(
        '/// When navigating to ${config.queryField} routes, pass the ${config.queryField}',
      );
    }
    ...
    // Build initial method call with parameters
    String initialMethodCall;
    if (config.methods.contains('getList')) {
      ...
      } else if (needsIdParam && config.queryField == config.idField) {
        initialMethodCall =
            'if (widget.id != null) {\n      controller.get$entityName(widget.id!);\n    }';
      } else if (needsQueryParam) {
        initialMethodCall =
            'if (widget.${config.queryField} != null) {\n      controller.get$entityName(widget.${config.queryField}!);\n    }';
      } else {
        initialMethodCall =
            '//TODO: pass the ${config.queryField} of the $entityName\n    //controller.get$entityName(/* ${config.queryField} */);';
      }
      ...
      if (config.queryFieldType == 'NoParams') {
        ...
        initialMethodCall =
            'if (widget.id != null) {\n      controller.watch$entityName(widget.id!);\n    }';
      } else if (needsQueryParam) {
        initialMethodCall =
            'if (widget.${config.queryField} != null) {\n      controller.watch$entityName(widget.${config.queryField}!);\n    }';
      } else {
        initialMethodCall =
            '//TODO: pass the ${config.queryField} of the $entityName\n    //controller.watch$entityName(/* ${config.queryField} */);';
      }
    } else {
      initialMethodCall = '// No initial method found';
    }
