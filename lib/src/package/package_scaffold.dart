...
class PackageScaffold {
  ...
  /// Resolves the hosted zuraffa constraint stamped into the generated
  /// pubspec + module. Injectable so tests can pin it deterministically;
  ...
  ///   (e.g. `^6.2.2`, stamped verbatim); by default it resolves to the
  ///   latest version published on pub.dev — a dev checkout's version
  ///   const is the *next unreleased* release and cannot resolve from a
  ///   clean machine (issue #1615). When the lookup fails the const is
  ///   used after a loud warning on stderr.
  ...
  Future<PackageScaffoldResult> create({
    ...
    // Resolved before any file write: the hosted constraint is the one
    // pub.dev actually serves, so the generated pubspec resolves from a
    // clean machine (issue #1615). An explicit [zuraffaConstraint]
    // short-circuits the lookup; a failed lookup warns loudly and falls
    // back to the running version const.
    final hostedConstraint = await resolveZuraffaConstraint(
    ...
    final files = <String, String>{
      ...
      p.join('lib', 'src', 'module', '${name}_package_module.dart'):
          _moduleStub(name, pascal, hostedConstraint),
...
''';
  ...
  String _moduleStub(String name, String pascal, String zuraffaConstraint) {
    return '''
...
class ${pascal}PackageModule extends PackageModule {
  ...
  /// version and rejects major mismatches with a clear error. Stamped from
  /// the published constraint the pubspec declares — a dev checkout's
  /// version const would make a generated package unloadable on a clean
  /// machine (issue #1615).
  @override
  String get zuraffaSdkConstraint => '$zuraffaConstraint';
