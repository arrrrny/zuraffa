...
import '../plugins/gym/capabilities/create_gym_capability.dart';

/// CLI command that generates GYM artifacts for existing entities/features.
///
/// Mirrors [TestCommand]: analyzes an existing usecase file (if present) to
/// infer dependencies, then delegates to [GymPlugin.generate] to emit the
/// `gym/` artifact (warmup reps + graded exercise + gym.yaml).
class GymCommand extends PluginCommand {
  @override
  final GymPlugin plugin;

  /// Creates a command bound to the provided [plugin].
  GymCommand(this.plugin) : super(plugin) {
    argParser.addOption(
      'domain',
      abbr: 'd',
      help: 'Domain folder for the gym target',
      defaultsTo: 'general',
    );
  }
  ...
  String get description => 'Generate Gym artifacts';
  ...
  Future<GeneratorResult> execute(
    ...
    if (args.isEmpty) {
      print('❌ Usage: zfa gym <Name> [options]');
