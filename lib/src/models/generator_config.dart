...
class GeneratorConfig {
  ...
  bool get appendToExisting => action == PluginAction.add; // Derived from action
  ...
  final bool force;
  final bool verbose;
  bool get revert => action == PluginAction.delete || action == PluginAction.remove; // Derived from action
  ...
  GeneratorConfig({
    ...
    this.outputDir = 'lib/src',
            ...
            (appendToExisting
                ? PluginAction.add
                : (revert ? PluginAction.delete : PluginAction.create)),
        queryFieldType = queryFieldType ?? 'String';

  factory GeneratorConfig.fromJson(Map<String, dynamic> json, String name) {
    // Legacy support for append/revert flags if action not provided
    PluginAction? action;
    if (json['action'] != null) {
      action = PluginAction.values.firstWhere(
        (e) => e.name == json['action'],
        orElse: () => PluginAction.create,
      );
    } else {
      if (json['append'] == true || json['append_to_existing'] == true) {
        action = PluginAction.add;
      } else if (json['revert'] == true) {
    ...
    return GeneratorConfig(
      name: json['name'] ?? name,
      action: action ?? PluginAction.create,
