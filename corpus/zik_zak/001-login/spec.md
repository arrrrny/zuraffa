...
The Login feature, as the ZikZak app ships it.
...
- **FR-001**: The system MUST save a Session when the user commits the Login form.
  traces: `SessionRepo.save`
- **FR-002**: The system MUST restore the last Login state on cold start.
  traces: `Hive.put`, `SessionRepo.get`

## Acceptance Scenarios

1. **Given** a signed-in user **When** they open Login **Then** the saved Session list renders.
   **Type**: widget
2. **Given** an empty local cache **When** Login loads **Then** the placeholder renders.
   **Type**: widget
