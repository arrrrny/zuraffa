...
The Deal Search feature, as the ZikZak app ships it.
...
- **FR-001**: The system MUST save a Bookmark when the user commits the Deal Search form.
  traces: `BookmarkRepo.save`
- **FR-002**: The system MUST restore the last Deal Search state on cold start.
  traces: `Hive.put`, `BookmarkRepo.get`

## Acceptance Scenarios

1. **Given** a signed-in user **When** they open Deal Search **Then** the saved Bookmark list renders.
   **Type**: widget
2. **Given** an empty local cache **When** Deal Search loads **Then** the placeholder renders.
   **Type**: widget
