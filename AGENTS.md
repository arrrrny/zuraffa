...
This document provides guidance for AI agents working with Zuraffa projects.

## Adding to Your Project's AGENTS.md

If you're using `zuraffa` in your project, add the following to your `AGENTS.md` or `CLAUDE.md` file to help AI agents understand and work with your codebase effectively.
...
This project uses `zuraffa` for Clean Architecture implementation.
...
Use the `zfa` CLI to generate boilerplate code:

### Entity-Based Generation

```bash
# Generate UseCases + Repository interface
zfa generate <EntityName> --methods=get,getList,create,update,delete --repository

# With VPC layer (View, Presenter, Controller)
zfa generate <EntityName> --methods=get,getList,create --repository --vpc

# With Data layer (DataRepository + DataSource)
zfa generate <EntityName> --methods=get,getList,create --repository --data
```
...
zfa generate <UseCaseName> \
  --repos=Repo1,Repo2 \
  --params=ParamsType \
  --returns=ReturnType

# Background UseCase (runs on isolate)
zfa generate <UseCaseName> --type=background --params=Params --returns=Result

# Stream UseCase
zfa generate <UseCaseName> --type=stream --repos=SomeRepo --returns=DataType
```
...
3. **Generate Presentation Layer**
   ```bash
   zfa generate Product --methods=get,getList,create --vpc --force
   ```
...
2. Run: `zfa generate {Name} --methods=get,getList,create,update,delete --repository --data --vpc`
