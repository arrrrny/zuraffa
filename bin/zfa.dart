#!/usr/bin/env dart
// ZFA CLI - Zuraffa Code Generator
//
...
''';
  ...
  Future<GeneratedFile> _generateState() async {
    ...
    // Determine if we need the list field (only for methods that work with lists)
    ...
    // Only add list field if needed by the selected methods
    if (needsListField) {
    ...
    final content = '''// Generated by zfa
...
${stateFields.join('\n')}
  ...
  $stateName copyWith({
    ...
    return $stateName(${_generateCopyWithBody(needsListField)});
  }
  ...
  bool operator ==(Object other) =>
      ...
      (other is $stateName &&
          runtimeType == other.runtimeType${_generateEqualityBody(needsListField)}${_generateEqualityChecks(stateFields)});

  @override
  int get hashCode =>
      ${_generateHashCodeBody(needsListField)};

  @override
  String toString() =>
      '${_generateToStringBody(needsListField, entityName)}';
}
''';
  ...
  /// Generate copyWith body with conditional list field
  String _generateCopyWithBody(bool needsListField) {
    final entityCamel = config.nameCamel;
    final parts = <String>[];
  ...
  /// Generate equality check body with conditional list field
  String _generateEqualityBody(bool needsListField) {
    final entityCamel = config.nameCamel;
    final parts = <String>[];
  ...
  /// Generate hashCode body with conditional list field
  String _generateHashCodeBody(bool needsListField) {
    final entityCamel = config.nameCamel;
    final parts = <String>[];

    if (needsListField) {
      parts.add('$entityCamel}List.hashCode');
    }

    parts.add('error.hashCode');

    // Add method-specific boolean fields
    for (final method in config.methods) {
      switch (method) {
        case 'get':
          parts.add('isGetting.hashCode');
          break;
        case 'getList':
          parts.add('isGettingList.hashCode');
          break;
        case 'create':
          parts.add('isCreating.hashCode');
          break;
        case 'update':
          parts.add('isUpdating.hashCode');
          break;
        case 'delete':
          parts.add('isDeleting.hashCode');
          break;
        case 'watch':
          parts.add('isWatching.hashCode');
          break;
        case 'watchList':
          parts.add('isWatchingList.hashCode');
          break;
      }
    }
  ...
  /// Generate toString body with conditional list field
  String _generateToStringBody(bool needsListField, String entityName) {
    final entityCamel = config.nameCamel;
    final stateName = '${entityName}State';

    if (needsListField) {
      return '$stateName($entityCamel}List: \${${entityCamel}List.length}, isLoading: \$isLoading, error: \$error)';
    } else {
      return '$stateName(isLoading: \$isLoading, error: \$error)';
    }
  }
