...
**Minimum viable:** Log a warning when "reused/reused" and binary mtime > stub mtime: `"stub is from an older binary; regenerate with --force to update"`. This at least tells the user why the regression is happening.
...
Found while running `zfa tdd run` on forklift spec 004 with a freshly rebuilt binary (fix #657 for non-entity behaviors). U1 gen returned "reused/reused", skipped the stub update, U1:make regressed. Manual deletion of the stub + resume fixed it.
