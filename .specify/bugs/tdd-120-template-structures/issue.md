# Bug Issue: [TDD-120] zfa parses the zuraffa template's declared structures (Key Entities table, Dependencies & Contracts, Layer Contracts, Template Version pin)
...
> Note: the raw issue body was stored JSON-escaped (literal `\n` sequences); it has been
> decoded to markdown below. Content is otherwise verbatim.
...
The template header carries `**Template Version**: `zuraffa-1.0``.
