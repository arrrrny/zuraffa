# Test List: cycle-log-phantom-sections
...
| -- | -------- | ------ | ----- |
| A1 | the in-fence `## ` lines do NOT start | AC-1 | PENDING |
| A2 | fence state stays synchronized and no `## ` line inside any | AC-2 | PENDING |
| A3 | the parsed sections are identical to what | AC-3 | PENDING |
| A4 | each sections the file through the shared | AC-4 | PENDING |
| A5 | it yields exactly one entry with the correct behavior id, kind, | AC-5 | PENDING |
