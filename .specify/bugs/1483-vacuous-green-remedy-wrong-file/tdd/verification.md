...
| 9 | Full chunked fast suite | 105-chunk list of `tools/run_tests_chunked.sh`, per-chunk runs, kernel caches cleared between chunks | PASS | 94/105 chunks green; ALL 11 non-passing chunks environmental (5 all-slow chunks → "No tests match the requested tag selectors"; 4 need the absent Flutter SDK → `flutter pub get: No such file or directory`; `test/commands` 280s green on a full window) |
...
- NOT RUN: the Flutter-SDK-dependent chunks (no Flutter SDK on this agent)
  and the all-slow presets beyond the suites listed in check #8; the full
