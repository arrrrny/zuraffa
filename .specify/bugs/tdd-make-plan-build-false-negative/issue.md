# Bug: fix(tdd): make plan includes func scaffold that races with pre-existing red tests — generation-error instead of skipped
...
## Report (verbatim from the GitHub issue)
...
Same root cause as #731 but in a different code path (the make plan's build step vs the direct make runner). This is a separate bug filing because the fix location is different: the plan-step runner vs the direct make runner.
