...
|----|----------|--------|------|-------|------|
| A-1472-1 | a refactor whose build pass is refused on warnings only completes through format+fix (outcome is never runner-error on 0 errors) | SC-1, SC-3 | cli | GREEN | `A-1472-1` in `test/plugins/tdd/bug_1472_refactor_gate_errors_only_test.dart` |
| A-1472-2 | a refactor whose build pass fails with analyzer errors still misfire-stops runner-error | SC-2 | cli | GREEN | `A-1472-2` in the same file |
