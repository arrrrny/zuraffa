...
- [ ] T1 (behaviour, SC-1) Tag `test/plugins/tdd/make_command_test.dart`
      `@Tags(['slow'])` → `@Tags(['regression', 'e2e'])`. Red: direct run
      exits 79 with 0 tests. Green: direct run exits 0 with 42 tests.
- [ ] T2 (behaviour, SC-2) Tag `test/plugins/tdd/make_command_declared_071_test.dart`
      `@Tags(['slow'])` → `@Tags(['regression', 'e2e'])`. Red: direct run
      exits 79 with 0 tests. Green: direct run exits 0 with 1 test.

## Selector / lane integrity

- [ ] T3 (non-behavioural, SC-3) `dart_test.yaml`: declare the `e2e:` tag
      in the `tags:` map and document it in the header comment.
- [ ] T4 (non-behavioural, SC-3) `.github/workflows/ci.yaml#dart_core`:
...
- [ ] T5 (behaviour, SC-4) Heavy lanes select the two files:
      `--preset=regression` and `--preset=all` both run them.
- [ ] T6 (behaviour, SC-5) CI-lane composition guard: the exact CI command
...
- [ ] T7 (non-behavioural, SC-6) `dart analyze` on all changed files: no
      new issues; `dart format` clean on changed Dart files.
- [ ] T8 (non-behavioural) Record red/green evidence in
