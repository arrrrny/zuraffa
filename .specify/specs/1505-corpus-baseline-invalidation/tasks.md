...
- [x] T001 (RED) Unit: a created file under `test/` flips the fingerprint;
      `read()` misses after the flip (US-1 / SC-1). Test before impl.
- [x] T002 (RED) Unit: a modified file under `lib/` flips the fingerprint
      (US-2 / SC-1). Test before impl.
- [x] T003 (RED) Unit: creating `.zfa/manifests/corpus-manifest.json` and
      changing `.zfa/context.json` each flip the fingerprint (US-3 / SC-2).
      Test before impl.
- [x] T004 (RED) Driver: after a test-file fix, the second feature's run
      does NOT report `corpus-wide reuse` and re-runs the live suite —
      the #1505 repro end-to-end (US-1 / SC-4). Test before impl.
- [x] T005 (GREEN) Implement: extend `dependencyFingerprint()` in
...
- [x] T006 (RED) Unit: mtime-only touch of a `test/` file does NOT flip
      the fingerprint (US-4 / SC-1, SC-3). Test before impl.
- [x] T007 (RED) Unit: run-mutated state (`.zfa/corpus/progress.json`,
      `.zfa/corpus/run.lock`, `.zfa/receipts/…`, the cache file itself)
      does NOT flip the fingerprint (US-4 / SC-3). Test before impl.
- [x] T008 (RED) Driver: with no outcome-relevant change, the second
...
- [x] T009 (GREEN) Implement: adjust only if T006–T008 exposed a leak
      (allow-list scope, marker semantics).

## Phase 3 — Non-behavioural

- [x] T010 Update the library doc comment and `dependencyFingerprint()`
      doc in `corpus_baseline_cache.dart` to name the new inputs and cite
      #1505 (docs only, no behaviour).
- [x] T011 `dart analyze` on changed files — no new warnings (SC-6).
- [x] T012 `dart format` the changed files; corpus_economics suite green.
- [x] T013 Write `tdd/verification.md` (red evidence, green evidence,
