...
- **Committed evidence**: the RED/GREEN/mutation captures live beside this
  file as `.txt` (`red-1538-fast.txt`, `red-1538-c1.txt`, `green-1538.txt`,
  `mutation-1538.txt`) — the repo's root `.gitignore` line 42 is `*.log`, so
  log-named evidence can never enter a PR. They were re-captured on this
  branch at the shipped commit so a reviewer can read the bytes rather than
  take them on trust; the lib diff is byte-identical to the one the audit
  describes.
...
| Test-first evidence | ✅ RED logs captured with the fix NOT yet written: `tdd/red-1538-fast.txt` (fast tier, exit 1 — A1/A2 fail: "does not contain 'Object? result;'" / "does not contain 'result = null;'") and `tdd/red-1538-c1.txt` (`--preset=all`, exit 1 — C1 fail with the pair's exact compile error) |
...
| Green | ✅ `tdd/green-1538.txt` — `--preset=all` (slow tier included), exit 0, `+6: All tests passed!` (A1/A2/A3/B1/B2/C1), captured AFTER `dart format` so the evidence matches the final bytes |
...
| Mutation testing (changed file) | ✅ 2/2 mutants killed (`tdd/mutation-1538.txt`): M1 void branch removed (revert mutant) → A1+A2 fail; M2 `result = null;` dropped → A1+A2 fail; pristine restored → all green. C1's kill of the revert mutant is additionally proven by the red-phase logs |
...
2. First run (fast tier): **A1 and A2 failed for the right reason** — the
   ...
   the chain. Evidence: `tdd/red-1538-fast.txt` (exit 1).
3. Slow tier (`--preset=all`): **C1 failed with the exact issue symptom** —
   ...
   Evidence: `tdd/red-1538-c1.txt` (exit 1).
4. Applied the single-point fix in `_captureInvocation` only: when
   ...
   bytes.
5. Suite went green: `+6: All tests passed!` (`tdd/green-1538.txt`,
   exit 0, `--preset=all`, post-format bytes).

## 2. Red → green (cycle log)

- **RED (evidence `tdd/red-1538-fast.txt`, `tdd/red-1538-c1.txt`):** exit 1 —
...
- **GREEN (evidence `tdd/green-1538.txt`):** exit 0 — `+6: All tests
...
|--------|--------|--------|
| M1 | void branch removed (behaviour reverts to the pre-fix IIFE for void returns) | KILLED — A1 (`does not contain 'Object? result;'`), A2 fail; C1 fails to load in the same reverted state (`red-1538-c1.txt` — the M1 case runs the fast tier, so C1 is not inside its own capture) |
| M2 | void branch emits `// mutant M2: completion not recorded` instead of `result = null;` | KILLED — A1/A2 fail on the missing completion record |
| Pristine | fix restored | all green |

Log: `tdd/mutation-1538.txt`.
...
- **SC-4** (the pair compiles and fails through an assertion) → C1:
  `dart test` on the emitted pair exits non-zero (honest red), names the
  emitted test in its output — the identity that proves an assertion ran,
  where a load failure reaches no reporter that names the test — and reports
  no `compile-time error` / `use_of_void_result` / `undefined name`. The
  proof deliberately does not read `Expected:`/`Actual:`: that wording is
  the reporter's, not the behavior's, and moves with `--reporter=json` and
  matcher versions.
